...
#[inline]
fn poly1305_do_32(k: &[u8], aadlen: u32, aad: &[u8], mlen: u32, m: &[u8], out: &mut [u8]) {
    let mut ctx: [u64; 25] = [0u64; 25usize];
    let mut block: [u8; 16] = [0u8; 16usize];
    crate::hacl::mac_poly1305::poly1305_init(&mut ctx, k);
    if aadlen != 0u32 {
        crate::hacl::aead_chacha20poly1305::poly1305_padded_32(&mut ctx, aadlen, aad)
    };
    if mlen != 0u32 {
        crate::hacl::aead_chacha20poly1305::poly1305_padded_32(&mut ctx, mlen, m)
    };
...
pub fn encrypt(
    ...
    crate::hacl::chacha20::chacha20_encrypt(64u32, &mut tmp, &tmp_copy, key, nonce, 0u32);
    let key1: (&[u8], &[u8]) = tmp.split_at(0usize);
    crate::hacl::aead_chacha20poly1305::poly1305_do_32(
        key1.1, data_len, data, input_len, output, tag,
    )
}
