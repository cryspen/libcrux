...
fn shake128_variable_out() {
    let (input_length_bits, tests) = parse_shake_variable_out_rsp(&tv_path("SHAKE128VariableOut.rsp"));
    assert!(!tests.is_empty());
    for (i, tc) in tests.iter().enumerate() {
...
fn shake256_variable_out() {
    let (input_length_bits, tests) = parse_shake_variable_out_rsp(&tv_path("SHAKE256VariableOut.rsp"));
    assert!(!tests.is_empty());
    for (i, tc) in tests.iter().enumerate() {
        let msg = &tc.msg[..input_length_bits / 8];
