...
--    A′[x,y] = A[x,y] ⊕ D[x]
--  θ sub-functions (extracted from theta body for cleaner verification)
def theta_c (state : RustArray u64 25) (x : usize) : RustM u64 := do
  let r ← get state x 0; let r ← r ^^^? (← get state x 1)
  let r ← r ^^^? (← get state x 2); let r ← r ^^^? (← get state x 3)
  r ^^^? (← get state x 4)

def theta_d (c : RustArray u64 5) (x : usize) : RustM u64 := do
  (← c[(← ((← (x +? (4 : usize))) %? (5 : usize)))]_?)
    ^^^? (← (core_models.num.Impl_9.rotate_left
      (← c[(← ((← (x +? (1 : usize))) %? (5 : usize)))]_?)
      (1 : u32)))

def theta_r (state : RustArray u64 25) (d : RustArray u64 5) (idx : usize) : RustM u64 := do
  (← state[idx]_?) ^^^? (← d[(← (idx /? (5 : usize)))]_?)

@[spec]
def theta (state : (RustArray u64 25)) : RustM (RustArray u64 25) := do
  let c ← hacspec_sha3.createi u64 5 (usize → RustM u64) (theta_c state)
  let d ← hacspec_sha3.createi u64 5 (usize → RustM u64) (theta_d c)
  hacspec_sha3.createi u64 25 (usize → RustM u64) (theta_r state d)
      ...
      (fun ⟨offset, output, state⟩ _squeeze_round =>
        ...
        let to_copy : usize ←
          if (← ((← (OUTPUT_LEN -? offset)) <? rate)) then do
            ...
            (pure rate);
        let output : (RustArray u8 OUTPUT_LEN) ←
          (squeeze_state state output offset to_copy);
