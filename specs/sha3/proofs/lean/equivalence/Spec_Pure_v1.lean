...
theorem xor_block_into_state_purifies
  ...
  · intro acc i hi
    ...
    -- Simplify the Option.bind/match from ExceptT desugaring
    simp only [ExceptT.bindCont, ExceptT.mk, Option.bind, ExceptT.pure, pure, Except.ok]
    -- Rewrite idx toNat to lane_index_pure value
    simp only [hidx_toNat]
    -- LHS has #v[block.val[8*i], ...][k] with Vector indexing.
    -- RHS has block.val[8*i.toNat + k]! with Array.get!.
    -- Step 1: Reduce Vector literal indexing #v[...][k] to the k-th element.
    -- Step 2: Show block.val[8*i+k] (USize64 index) = block.val[8*i.toNat+k]! (Nat index + get!).
    -- Both follow because (8*i+k).toNat = 8*i.toNat+k and indices are in bounds.
    congr 1; congr 1; congr 1
    · rfl
    · congr 1
      · rfl
      · congr 1 <;> simp [h8i_toNat, h8i_block]
