...
fn same_inputs_result_in_same_output() {
    ...
    let (libcrux_key_pair, spec_key_pair) = loop {
        ...
        if libcrux_key_pair_result.is_ok() {
            break (
                libcrux_key_pair_result.unwrap(),
                spec_key_pair_result.unwrap(),
            );
        } else {
            spec_key_pair_result.expect_err(
                "if rejection sampling failed in libcrux, it should've failed in the spec as well.",
            );
        }
    };
    ...
    let (libcrux_ct, libcrux_ss) =
        libcrux::kem::kyber768_encapsulate_derand(&libcrux_key_pair.pk().into(), message).unwrap();
    ...
    let libcrux_ss =
        libcrux::kem::kyber768_decapsulate_derand(libcrux_key_pair.private_key(), &libcrux_ct);
...
fn modify_ciphertext_pair(
    libcrux_ct: KyberCiphertext<KYBER768_CIPHERTEXT_SIZE>,
    mut spec_ct: hacspec_kyber::Ciphertext,
) -> (
    KyberCiphertext<KYBER768_CIPHERTEXT_SIZE>,
...
fn implicit_rejection_happens_the_same_way() {
    ...
    let libcrux_ss = libcrux::kem::kyber768_decapsulate_derand(
