...
fn same_inputs_result_in_same_output() {
    ...
    let (libcrux_key_pair, spec_key_pair) = loop {
        ...
        if libcrux_key_pair_result.is_ok() {
            break (libcrux_key_pair_result.unwrap(), spec_key_pair_result.unwrap())
        } else {
            spec_key_pair_result.expect_err("if rejection sampling failed in libcrux, it should've failed in the spec as well.");
        }
    };
