...
use self::kyber_polynomial_ring_element_mod::multiply_ntts;
...
pub(crate) mod kyber_polynomial_ring_element_mod {
    ...
    pub fn ntt(f: KyberPolynomialRingElement) -> KyberPolynomialRingElement {
        let mut f_hat = f;
        let mut k: u8 = 1;
        ...
        for len in NTT_LAYERS.iter().rev() {
            ...
            for start in (0..(COEFFICIENTS_IN_RING_ELEMENT - len)).step_by(2 * len) {
                // zeta ← ζ^(BitRev₇(k)) mod q
                // k ← k + 1
                let zeta = ZETA.pow(reverse_as_7_bit_number(k));
                k += 1;
    ...
    /// The NIST FIPS 203 standard can be found at
    /// <https://csrc.nist.gov/pubs/fips/203/ipd>.
    pub fn ntt_inverse(f_hat: KyberPolynomialRingElement) -> KyberPolynomialRingElement {
        let mut f = f_hat;
        let mut k: u8 = 127;
    ...
    mod tests {
        ...
        fn seven_bit_reverse() {
            assert_eq!(64, reverse_as_7_bit_number(1));
            assert_eq!(127, reverse_as_7_bit_number(255));
            assert_eq!(78, reverse_as_7_bit_number(185));
        }
    }
}
...
mod tests {
    ...
    use crate::{
        ...
        ntt::kyber_polynomial_ring_element_mod::{ntt, ntt_inverse},
    };
