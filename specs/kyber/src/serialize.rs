...
///
pub(crate) fn deserialize_ring_element(ring_element_bytes : [u8; parameters::BITS_PER_RING_ELEMENT / 8]) -> RingElement {
    ...
    for i in 0..ring_element.coefficients.len() {
        let coefficient = ring_coefficient_bits_as_u16(&ring_element_bits[i*parameters::BITS_PER_COEFFICIENT..(i + 1)*parameters::BITS_PER_COEFFICIENT].try_into().unwrap());
...
///
pub(crate) fn serialize_ring_element(ring_element : &RingElement) -> [u8; parameters::BITS_PER_RING_ELEMENT / 8] {
    ...
    for i in 0..serialized.len() {
        serialized[i] = bit_vector_as_u8(&ring_element_bits[i*8..(i+1)*8].try_into().unwrap());
    }
    serialized
}
