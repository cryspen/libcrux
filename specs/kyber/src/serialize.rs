...
impl KyberPolynomialRingElement {
    ...
    pub fn serialize_little_endian(&self, bits_per_coefficient: usize) -> Vec<u8> {
        ...
        for bit_vector in self
            .to_little_endian_bit_vector(bits_per_coefficient)
            .chunks(8)
        {
    ...
    pub fn deserialize_little_endian(
        ...
        for i in 0..deserialized.coefficients.len() {
            deserialized.coefficients[i] = Self::field_element_from_little_endian_bit_vector(
                ring_element_bits.next().unwrap(),
            );
        }
...
mod tests {
    ...
    proptest! {
        ...
        fn deserialize_is_left_inverse_of_serialize_when_no_compression(ring_element in arb_ring_element(12)) {
            let ring_element_serialized = ring_element.serialize_little_endian(12);
            assert_eq!(ring_element, KyberPolynomialRingElement::deserialize_little_endian(12, &ring_element_serialized));
        }

        #[test]
        fn serialize_is_sometimes_left_inverse_of_deserialize_when_no_compression(ring_element in arb_ring_element(12)) {
            let ring_element_serialized = ring_element.serialize_little_endian(12);
            assert_eq!(ring_element_serialized, KyberPolynomialRingElement::deserialize_little_endian(12, &ring_element_serialized).serialize_little_endian(12));
        }
    }
}
