...
(*** Phase 7b — Forward NTT layer commute + Phase 7a inverse NTT layer 1 ***)

(* Function-form per-vector hacspec bridges (forward layer 1 +
   inverse layer 1 + helpers `mont_array_lane` / `zetas_4_lane` /
   `lemma_ntt_layer_n_16_2_lane` / `lemma_ntt_inverse_layer_n_16_2_lane`)
   were moved out to a sibling module:

       Hacspec_ml_kem.Commute.Bridges

   Reason: the lane-bridge queries are slow without Z3 hints (multi-
   minute searches on first verification).  Splitting them out keeps
   `Chunk.fst` fast on every iteration; consumers that need the
   bridges open `Hacspec_ml_kem.Commute.Bridges` instead of importing
   them through `Chunk`.

   `Chunk.fst` retains:
     - per-pair commute helpers (`lemma_butterfly_pair_commute`,
       `lemma_inv_butterfly_pair_commute`, `lemma_ntt_layer_1_butterfly_to_fe`,
       `lemma_inv_ntt_layer_1_butterfly_to_fe`)
     - trivial `_chunk_commutes` re-statements (referenced from
       `polynomial.rs` and `avx2.rs`)
     - Phase 7a Tier-1 polynomial commute lemmas
     - Phase 7b INTT-Mont foundation (`intt_mont_form_lane`,
       `lemma_intt_mont_finalize_fe`, etc.)

   Future per-vector function-form bridges for layers 2/3 and
   layer-4_plus go in `Bridges.fst`, NOT here. *)

[@@ noextract_to "krml"]
let _placeholder_section_moved_to_bridges = ()
