...
use crate::parameters::{bitlen, MlDsaParams, Polynomial, D, Q, ZERO_POLY};
...
pub fn simple_bit_pack<const BYTES: usize>(w: &Polynomial, b: usize) -> [u8; BYTES] {
    let bits = bitlen(b);
...
pub fn bit_pack<const BYTES: usize>(w: &Polynomial, a: usize, b: usize) -> [u8; BYTES] {
    let bits = bitlen(a + b);
    let mut out = [0u8; BYTES];
    for i in 0usize..256usize {
        hax_lib::loop_invariant!(|i: usize| i <= 256usize);
...
pub fn coeff_from_half_byte(b: u8, eta: usize) -> Option<i32> {
