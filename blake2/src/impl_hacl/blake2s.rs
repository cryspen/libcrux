...
impl<const OUT_SIZE: usize> libcrux_traits::digest::arrayref::DigestIncremental<OUT_SIZE>
    for Blake2s<ConstKeyLenConstDigestLen<0, OUT_SIZE>>
{
...
pub type Blake2sHasher<const N: usize> =
    libcrux_traits::digest::Hasher<N, Blake2s<ConstKeyLenConstDigestLen<0, N>>>;
pub type Blake2sSliceHasher = libcrux_traits::digest::SliceHasher<Blake2s<ConstKeyLen<0>>>;

impl<const N: usize> From<Blake2s<ConstKeyLenConstDigestLen<0, N>>> for Blake2sHasher<N> {
    fn from(hasher: Blake2s<ConstKeyLenConstDigestLen<0, N>>) -> Self {
        Self {
            state: hasher.state,
        }
    }
}
impl From<Blake2s<ConstKeyLen<0>>> for Blake2sSliceHasher {
    fn from(hasher: Blake2s<ConstKeyLen<0>>) -> Self {
        Self {
            state: hasher.state,
        }
    }
...
impl libcrux_traits::digest::slice::DigestIncremental for Blake2s<ConstKeyLen<0>> {
    type IncrementalState = Box<[state_t]>;
