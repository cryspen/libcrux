  ...
  let v_A_transpose, sampling_A_error:(array
    ...
    Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter ({
      ...
      (fun (v_A_transpose, sampling_A_error) i ->
          let seeds:array (array u8 34sz) v_K = Rust_primitives.Hax.repeat seed v_K in
          let seeds:array (array u8 34sz) v_K =
            Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter
                  ({ Core.Ops.Range.Range.f_start = 0sz; Core.Ops.Range.Range.f_end = v_K })
                <:
                _)
              seeds
              (fun seeds j ->
                  let seeds:array (array u8 34sz) v_K =
                    Rust_primitives.Hax.update_at seeds
                      j
                      (Rust_primitives.Hax.update_at (seeds.[ j ] <: array u8 34sz) 32sz (cast i)
                        <:
                        array u8 34sz)
                  in
                  let seeds:array (array u8 34sz) v_K =
                    Rust_primitives.Hax.update_at seeds
                      j
                      (Rust_primitives.Hax.update_at (seeds.[ j ] <: array u8 34sz) 33sz (cast j)
                        <:
                        array u8 34sz)
                  in
                  seeds)
          in
          let xof_bytes:array (array u8 840sz) v_K =
...
let cbd (#k #eta #eta_randomness_size: usize) (prf_input: array u8 33sz)
...
let generate_keypair
      (#k #private_key_size #public_key_size #bytes_per_ring_element #eta1 #eta1_randomness_size:
          usize)
...
let encrypt
      (#k #ciphertext_size #t__as_ntt_encoded_size #c1_len #c2_len #vector_u_compression_factor #vector_v_compression_factor #block_len #eta1 #eta1_randomness_size #eta2 #eta2_randomness_size:
          usize)
      (public_key: slice u8)
