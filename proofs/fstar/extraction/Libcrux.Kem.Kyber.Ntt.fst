      ...
      (fun (re, zeta_i) layer ->
          Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter (Core.Iter.Traits.Iterator.Iterator.step_by
                    ({
                        ...
                        =
                        Libcrux.Kem.Kyber.Constants.v_COEFFICIENTS_IN_RING_ELEMENT -. layer <: _
                      })
                    (2sz *. layer <: _)
                  <:
            ...
            (fun (re, zeta_i) offset ->
                ...
                let re:Libcrux.Kem.Kyber.Arithmetic.t_KyberPolynomialRingElement =
                  Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter
                    ...
                    (fun re j ->
                        let t:i32 =
                          Libcrux.Kem.Kyber.Arithmetic.montgomery_reduce ((re.[ j +. layer <: _ ]
                                <:
                                i32) *.
                ...
                re, zeta_i)
          <:
          (Libcrux.Kem.Kyber.Arithmetic.t_KyberPolynomialRingElement & Prims.unit))
  in
  let re:Libcrux.Kem.Kyber.Arithmetic.t_KyberPolynomialRingElement =
    {
      re with
      ...
      (fun (re, zeta_i) layer ->
          Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter (Core.Iter.Traits.Iterator.Iterator.step_by
                    ({
                        ...
                        Core.Ops.Range.Range.f_end
                        =
                        Libcrux.Kem.Kyber.Constants.v_COEFFICIENTS_IN_RING_ELEMENT -. layer <: usize
                      })
                    (2sz *. layer <: usize)
                  <:
                  Core.Iter.Adapters.Step_by.t_StepBy (Core.Ops.Range.t_Range usize))
  ...
  let re:Libcrux.Kem.Kyber.Arithmetic.t_KyberPolynomialRingElement =
    {
      ...
      Core.Array.map_under_impl_23 (Core.Array.map_under_impl_23 re
              .Libcrux.Kem.Kyber.Arithmetic.KyberPolynomialRingElement.f_coefficients
            (fun coefficient ->
                Libcrux.Kem.Kyber.Arithmetic.montgomery_reduce (coefficient *. 1441l <: i32) <: i32)
          <:
          array i32 256sz)
        Libcrux.Kem.Kyber.Arithmetic.barrett_reduce
    }
