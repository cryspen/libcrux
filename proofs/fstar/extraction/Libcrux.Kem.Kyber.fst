...
let generate_keypair
  ...
  let secret_key_serialized:t_Array u8 v_PRIVATE_KEY_SIZE =
    Libcrux.Kem.Kyber.Ind_cpa.serialize_secret_key (Rust_primitives.unsize (Libcrux.Kem.Kyber.Types.impl_40__as_slice
              ind_cpa_private_key
            <:
            t_Array u8 v_CPA_PRIVATE_KEY_SIZE)
        <:
        t_Slice u8)
      (Rust_primitives.unsize (Libcrux.Kem.Kyber.Types.impl_30__as_slice public_key
            <:
  ...
  let to_hash:t_Array u8 (sz 64) =
    Rust_primitives.Hax.update_at to_hash
      ...
      (Core.Slice.impl__copy_from_slice (Core.Ops.Index.IndexMut.index_mut to_hash
            ...
            <:
            t_Slice u8)
          (Rust_primitives.unsize (Libcrux.Kem.Kyber.Hash_functions.v_H (Rust_primitives.unsize (Libcrux.Kem.Kyber.Types.impl_30__as_slice
                          public_key
                        <:
                        t_Array u8 v_PUBLIC_KEY_SIZE)
                    <:
...
let decapsulate
  ...
  let ind_cpa_secret_key, secret_key:(t_Slice u8 & t_Slice u8) =
    Libcrux.Kem.Kyber.Types.impl_20__split_at secret_key v_CPA_SECRET_KEY_SIZE
  in
