...
let generate_keypair
  ...
  let secret_key_serialized:t_Array u8 v_PRIVATE_KEY_SIZE =
    Libcrux.Kem.Kyber.Ind_cpa.serialize_secret_key (Rust_primitives.unsize (Libcrux.Kem.Kyber.Types.impl_50__as_slice
              ind_cpa_private_key
            <:
            t_Array u8 v_CPA_PRIVATE_KEY_SIZE)
        <:
        t_Slice u8)
      (Rust_primitives.unsize (Libcrux.Kem.Kyber.Types.impl_40__as_slice public_key
            <:
...
let encapsulate
  ...
  let ciphertext, sampling_a_error:(Libcrux.Kem.Kyber.Types.t_KyberCiphertext v_CIPHERTEXT_SIZE &
    Core.Option.t_Option Libcrux.Kem.Kyber.Types.t_Error) =
    Libcrux.Kem.Kyber.Ind_cpa.encrypt (Rust_primitives.unsize (Libcrux.Kem.Kyber.Types.impl_40__as_slice
              public_key
            <:
            t_Array u8 v_PUBLIC_KEY_SIZE)
        <:
...
let decapsulate
  ...
  let ind_cpa_secret_key, secret_key:(t_Slice u8 & t_Slice u8) =
    Libcrux.Kem.Kyber.Types.impl_30__split_at secret_key v_CPA_SECRET_KEY_SIZE
  in
