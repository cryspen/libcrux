...
let v_ETA1_RANDOMNESS_SIZE: usize = normalize_term (v_ETA1 *! sz 64)

let v_ETA2: usize = sz 2

let v_ETA2_RANDOMNESS_SIZE: usize = normalize_term (v_ETA2 *! sz 64)

let v_RANK_768_: usize = sz 3

let v_CPA_PKE_SECRET_KEY_SIZE_768_: usize = normalize_term (
...
let v_RANKED_BYTES_PER_RING_ELEMENT_768_: usize = normalize_term (
  (v_RANK_768_ *! Libcrux.Kem.Kyber.Constants.v_BITS_PER_RING_ELEMENT <: usize) /! sz 8)

let v_T_AS_NTT_ENCODED_SIZE_768_: usize = normalize_term (
  ((v_RANK_768_ *! Libcrux.Kem.Kyber.Constants.v_COEFFICIENTS_IN_RING_ELEMENT <: usize) *!
    Libcrux.Kem.Kyber.Constants.v_BITS_PER_COEFFICIENT
    <:
    usize) /!
...
let v_CPA_PKE_PUBLIC_KEY_SIZE_768_: usize = normalize_term (v_T_AS_NTT_ENCODED_SIZE_768_ +! sz 32)

let v_SECRET_KEY_SIZE_768_: usize = normalize_term (
...
let v_C1_BLOCK_SIZE_768_: usize = normalize_term (
  (Libcrux.Kem.Kyber.Constants.v_COEFFICIENTS_IN_RING_ELEMENT *! v_VECTOR_U_COMPRESSION_FACTOR_768_
    <:
    usize) /!
  sz 8)

let v_C1_SIZE_768_: usize = normalize_term (v_C1_BLOCK_SIZE_768_ *! v_RANK_768_)

let v_VECTOR_V_COMPRESSION_FACTOR_768_: usize = sz 4

let v_C2_SIZE_768_: usize = normalize_term (
  (Libcrux.Kem.Kyber.Constants.v_COEFFICIENTS_IN_RING_ELEMENT *! v_VECTOR_V_COMPRESSION_FACTOR_768_
    <:
    usize) /!
  sz 8)

let v_CPA_PKE_CIPHERTEXT_SIZE_768_: usize = normalize_term (v_C1_SIZE_768_ +! v_C2_SIZE_768_)

let v_IMPLICIT_REJECTION_HASH_INPUT_SIZE: usize = normalize_term (
...
let encapsulate_768_
    ...
    (Core.Result.t_Result (Libcrux.Kem.Kyber.Types.t_KyberCiphertext (sz 1088) & t_Array u8 (sz 32))
