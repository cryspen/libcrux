...
pub use matrix::{
    compute_As_plus_e, compute_message, compute_ring_element_v, compute_vector_u, sample_matrix_A,
};
pub use ntt::{invert_ntt_montgomery, ntt_binomially_sampled_ring_element, ntt_vector_u};
pub use sampling::sample_from_binomial_distribution;
pub use serialize::{
...
pub(crate) const VECTORS_IN_RING_ELEMENT: usize =
    COEFFICIENTS_IN_RING_ELEMENT / traits::FIELD_ELEMENTS_IN_VECTOR;
...
#[inline(always)]
pub fn from_i32_array(a: [i32; 256]) -> PolynomialRingElement {
...
#[inline(always)]
pub fn add_to_ring_element<const K: usize>(
    mut lhs: PolynomialRingElement,
    rhs: &PolynomialRingElement,
) -> PolynomialRingElement {
    for i in 0..lhs.coefficients.len() {
        lhs.coefficients[i] = Vector::add(lhs.coefficients[i], &rhs.coefficients[i]);
    }
    lhs
}

#[inline(always)]
pub fn poly_barrett_reduce(mut a: PolynomialRingElement) -> PolynomialRingElement {
    for i in 0..VECTORS_IN_RING_ELEMENT {
        a.coefficients[i] = Vector::barrett_reduce(a.coefficients[i]);
    }
    a
}

#[inline(always)]
pub fn subtract_reduce(
    ...
    for i in 0..VECTORS_IN_RING_ELEMENT {
        ...
        b.coefficients[i] =
            Vector::barrett_reduce(Vector::sub(a.coefficients[i], &coefficient_normal_form));
    }
    b
}
...
pub fn add_message_error_reduce(
    ...
    for i in 0..VECTORS_IN_RING_ELEMENT {
        let coefficient_normal_form =
            Vector::montgomery_reduce(Vector::multiply_by_constant(result.coefficients[i], 1441));
        result.coefficients[i] = Vector::barrett_reduce(Vector::add(
            coefficient_normal_form,
            &Vector::add(err.coefficients[i], &message.coefficients[i]),
        ));
    }
    result
}
...
#[inline(always)]
pub fn add_error_reduce(
    err: &PolynomialRingElement,
    mut result: PolynomialRingElement,
) -> PolynomialRingElement {
    for j in 0..VECTORS_IN_RING_ELEMENT {
        let coefficient_normal_form =
            Vector::montgomery_reduce(Vector::multiply_by_constant(result.coefficients[j], 1441));

        result.coefficients[j] =
            Vector::barrett_reduce(Vector::add(coefficient_normal_form, &err.coefficients[j]));
    }
    result
}

#[inline(always)]
pub fn add_standard_error_reduce(
    err: &PolynomialRingElement,
    mut result: PolynomialRingElement,
) -> PolynomialRingElement {
    for j in 0..VECTORS_IN_RING_ELEMENT {
...
const ZETAS_TIMES_MONTGOMERY_R: [i32; 128] = [
...
#[inline(always)]
fn ntt_layer_int_vec_step(mut a: Vector, mut b: Vector, zeta_r: i32) -> (Vector, Vector) {
...
#[inline(always)]
fn ntt_layer_7_int_vec_step(mut a: Vector, mut b: Vector) -> (Vector, Vector) {
...
#[inline(always)]
pub fn ntt_at_layer_7(mut re: PolynomialRingElement) -> PolynomialRingElement {
...
#[inline(always)]
fn inv_ntt_layer_int_vec_step(mut a: Vector, mut b: Vector, zeta_r: i32) -> (Vector, Vector) {
...
#[inline(always)]
pub fn invert_ntt_at_layer_3_plus(
    zeta_i: &mut usize,
    mut re: PolynomialRingElement,
    layer: usize,
) -> PolynomialRingElement {
