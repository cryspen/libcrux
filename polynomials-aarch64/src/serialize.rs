...
pub fn compress_then_serialize_message(re: PolynomialRingElement) -> [u8; SHARED_SECRET_SIZE] {
    ...
    for i in 0..32 {
        ...
        let coefficient_compressed = crate::Vector::compress_1(coefficient);
        serialized[i] = crate::Vector::serialize_1(coefficient_compressed);
    }
    serialized
}
#[inline(always)]
pub fn deserialize_then_decompress_message(
    ...
    for i in 0..32 {
        let coefficient_compressed = crate::Vector::deserialize_1(serialized[i]);
        re.coefficients[i] = crate::Vector::decompress_1(coefficient_compressed);
    }
    re
}
...
pub fn serialize_uncompressed_ring_element(
    ...
    for i in 0..VECTORS_IN_RING_ELEMENT {
        let coefficient = crate::Vector::to_unsigned_representative(re.coefficients[i]);
        let bytes = crate::Vector::serialize_12(coefficient);
...
fn compress_then_serialize_10<const OUT_LEN: usize>(re: PolynomialRingElement) -> [u8; OUT_LEN] {
    ...
    for i in 0..VECTORS_IN_RING_ELEMENT {
        let coefficient = crate::Vector::compress::<10>(crate::Vector::to_unsigned_representative(
            re.coefficients[i],
        ));
        let bytes = crate::Vector::serialize_10(coefficient);
...
fn compress_then_serialize_11<const OUT_LEN: usize>(re: PolynomialRingElement) -> [u8; OUT_LEN] {
    let mut serialized = [0u8; OUT_LEN];
    for i in 0..VECTORS_IN_RING_ELEMENT {
        let coefficient = crate::Vector::compress::<11>(crate::Vector::to_unsigned_representative(
            re.coefficients[i],
        ));
        let bytes = crate::Vector::serialize_11(coefficient);
...
fn compress_then_serialize_4<const OUT_LEN: usize>(re: PolynomialRingElement) -> [u8; OUT_LEN] {
    let mut serialized = [0u8; OUT_LEN];
    for i in 0..VECTORS_IN_RING_ELEMENT {
        let coefficient = crate::Vector::compress::<4>(crate::Vector::to_unsigned_representative(
            re.coefficients[i],
        ));
        let bytes = crate::Vector::serialize_4(coefficient);
...
fn compress_then_serialize_5<const OUT_LEN: usize>(re: PolynomialRingElement) -> [u8; OUT_LEN] {
    ...
    for i in 0..VECTORS_IN_RING_ELEMENT {
        let coefficients = crate::Vector::compress::<5>(crate::Vector::to_unsigned_representative(
            re.coefficients[i],
        ));
        let bytes5 = crate::Vector::serialize_5(coefficients);
