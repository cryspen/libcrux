...
let compress_message_coefficient (fe: u16)
    : Prims.Pure u8
      (requires fe <. (cast (Libcrux_traits.v_FIELD_MODULUS <: i16) <: u16))
      (ensures
        fun result ->
...
let compress_ciphertext_coefficient (coefficient_bits: u8) (fe: u16)
    : Prims.Pure i16
      (requires
        ...
        fe <. (cast (Libcrux_traits.v_FIELD_MODULUS <: i16) <: u16))
      (ensures
        fun result ->
...
let montgomery_reduce_element (value: i32)
    : Prims.Pure i16
      (requires
        value >=.
        ((cast (Core.Ops.Arith.Neg.neg Libcrux_traits.v_FIELD_MODULUS <: i16) <: i32) *!
          v_MONTGOMERY_R
          <:
          i32) &&
        value <=. ((cast (Libcrux_traits.v_FIELD_MODULUS <: i16) <: i32) *! v_MONTGOMERY_R <: i32))
      (ensures
        fun result ->
          let result:i16 = result in
          result >=.
          ((Core.Ops.Arith.Neg.neg (3s *! Libcrux_traits.v_FIELD_MODULUS <: i16) <: i16) /! 2s
            <:
            i16) &&
          result <=. ((3s *! Libcrux_traits.v_FIELD_MODULUS <: i16) /! 2s <: i16)) =
...
let rej_sample (a: t_Slice u8) : (usize & t_Array i16 (sz 16)) =
  let result:t_Array i16 (sz 16) = Rust_primitives.Hax.repeat 0s (sz 16) in
  let sampled:usize = sz 0 in
  let result, sampled:(t_Array i16 (sz 16) & usize) =
    Core.Iter.Traits.Iterator.f_fold (Core.Iter.Traits.Collect.f_into_iter (Core.Slice.impl__chunks a
        ...
        <:
        Core.Slice.Iter.t_Chunks u8)
      (result, sampled <: (t_Array i16 (sz 16) & usize))
      (fun temp_0_ bytes ->
          let result, sampled:(t_Array i16 (sz 16) & usize) = temp_0_ in
          ...
          let result, sampled:(t_Array i16 (sz 16) & usize) =
            ...
            then
              let result:t_Array i16 (sz 16) =
                Rust_primitives.Hax.Monomorphized_update_at.update_at_usize result sampled d1
              in
              result, sampled +! sz 1 <: (t_Array i16 (sz 16) & usize)
            else result, sampled <: (t_Array i16 (sz 16) & usize)
          in
  ...
  in
  sampled, result <: (usize & t_Array i16 (sz 16))
...
let from_i16_array (array: t_Array i16 (sz 16)) : t_PortableVector =
  { f_elements = array } <: t_PortableVector
...
let decompress (v_COEFFICIENT_BITS: i32) (v: t_PortableVector) : t_PortableVector =
  let _:Prims.unit =
    if true
    then
...
let impl: Libcrux_traits.t_Operations t_PortableVector =
  {
    ...
    f_from_i16_array_post = (fun (array: t_Array i16 (sz 16)) (out: t_PortableVector) -> true);
    f_from_i16_array = (fun (array: t_Array i16 (sz 16)) -> from_i16_array array);
    ...
    f_decompress_pre = (fun (v_COEFFICIENT_BITS: i32) (v: t_PortableVector) -> true);
    f_decompress_post
    =
    (fun (v_COEFFICIENT_BITS: i32) (v: t_PortableVector) (out: t_PortableVector) -> true);
    f_decompress
    =
    (fun (v_COEFFICIENT_BITS: i32) (v: t_PortableVector) -> decompress v_COEFFICIENT_BITS v);
    ...
    f_rej_sample = fun (a: t_Slice u8) -> rej_sample a
  }
