...
fn nym_outfox_create(c: &mut Criterion) {
    ...
    group.bench_function("libcrux", |b| {
        b.iter_batched(
            ...
            |(pk1, sk2a, sk2b, sk2c, sk2d)| {
                let pk2a = ecdh::secret_to_public(ecdh::Algorithm::X25519, &sk2a).unwrap();
                let pk2b = ecdh::secret_to_public(ecdh::Algorithm::X25519, &sk2b).unwrap();
                let pk2c = ecdh::secret_to_public(ecdh::Algorithm::X25519, &sk2c).unwrap();
                let pk2d = ecdh::secret_to_public(ecdh::Algorithm::X25519, &sk2d).unwrap();
    ...
    group.bench_function("Ring", |b| {
        ...
        b.iter_batched(
            ...
            |(pk1, sk2a, sk2b, sk2c, sk2d)| {
                let pk2a = sk2a.compute_public_key().unwrap();
                let pk2b = sk2b.compute_public_key().unwrap();
                let pk2c = sk2c.compute_public_key().unwrap();
                let pk2d = sk2d.compute_public_key().unwrap();
   ...
   group.bench_function("Dalek", |b| {
        ...
        b.iter_batched(
            ...
            |(pk1, sk2a, sk2b, sk2c, sk2d)| {
	    	let pk2a = PublicKey::from(&sk2a);
	    	let pk2b = PublicKey::from(&sk2b);
	    	let pk2c = PublicKey::from(&sk2c);
	    	let pk2d = PublicKey::from(&sk2d);
    ...
    group.bench_function("Dalek Ristretto", |b| {
        ...
        b.iter_batched(
            ...
            |(pk1, sk2a, sk2b, sk2c, sk2d)| {
	    	let pk2a = RistrettoPoint::mul_base(&sk2a);
	    	let pk2b = RistrettoPoint::mul_base(&sk2b);
	    	let pk2c = RistrettoPoint::mul_base(&sk2c);
	    	let pk2d = RistrettoPoint::mul_base(&sk2d);
                let _zza = pk1 * sk2a;
                let _zzb = pk1 * sk2b;
                let _zzc = pk1 * sk2c;
                let _zzd = pk1 * sk2d;
    ...
    group.bench_function("lib25519", |b| {
		...
		unsafe {
		  lib25519::lib25519_nPbatch_montgomery25519(zzs.as_mut_ptr(),pks.as_ptr(),sks.as_ptr(),8);
		}
            },
            BatchSize::SmallInput,
        )
...
fn nym_sphinx_create(c: &mut Criterion) {
    ...
    group.bench_function("libcrux", |b| {
        b.iter_batched(
            ...
            |(pk1, sk2)| {
                let pk2a = ecdh::secret_to_public(ecdh::Algorithm::X25519, &sk2).unwrap();
                let zza = ecdh::derive(ecdh::Algorithm::X25519, &pk1, &sk2).unwrap();
                let pk2b = ecdh::secret_to_public(ecdh::Algorithm::X25519, &zza).unwrap();
                let zzb = ecdh::derive(ecdh::Algorithm::X25519, &pk1, &zza).unwrap();
                let pk2c = ecdh::secret_to_public(ecdh::Algorithm::X25519, &zzb).unwrap();
                let zzc = ecdh::derive(ecdh::Algorithm::X25519, &pk1, &zzb).unwrap();
                let pk2d = ecdh::secret_to_public(ecdh::Algorithm::X25519, &zzc).unwrap();
...
criterion_group!(benches, derive, secret_to_public, nym_outfox_create, nym_outfox_process, nym_sphinx_create, nym_sphinx_process);
