...
pub fn update(
    ...
    if chunk_len as u64 > 0xffffffffu64.wrapping_sub(total_len) {
        ...
        let sz: u32 = if total_len.wrapping_rem(16u32 as u64) == 0u64 && total_len > 0u64 {
            16u32
        } else {
        ...
        if chunk_len <= 16u32.wrapping_sub(sz) {
            ...
            if sz1 != 0u32 {
                crate::hacl::mac_poly1305::poly1305_update(block_state, 16u32, buf)
            };
            let ite: u32 = if (chunk_len as u64).wrapping_rem(16u32 as u64) == 0u64
            ...
            crate::hacl::mac_poly1305::poly1305_update(block_state, data1_len, data2.0);
            ...
            if sz10 != 0u32 {
                crate::hacl::mac_poly1305::poly1305_update(block_state, 16u32, buf0)
            };
            let ite: u32 = if (chunk_len.wrapping_sub(diff) as u64).wrapping_rem(16u32 as u64)
                == 0u64
...
pub fn digest(state: &[crate::hacl::mac_poly1305::state_t], output: &mut [u8]) {
    ...
    let r: u32 = if total_len.wrapping_rem(16u32 as u64) == 0u64 && total_len > 0u64 {
        16u32
    } else {
        total_len.wrapping_rem(16u32 as u64) as u32
    };
    ...
    let ite: u32 = if r.wrapping_rem(16u32) == 0u32 && r > 0u32 {
        16u32
    } else {
        r.wrapping_rem(16u32)
    };
    let buf_last: (&[u8], &[u8]) = buf_multi.1.split_at(r.wrapping_sub(ite) as usize);
    crate::hacl::mac_poly1305::poly1305_update(tmp_block_state, 0u32, buf_last.0);
    crate::hacl::mac_poly1305::poly1305_update(tmp_block_state, r, buf_last.1);
    let mut tmp: [u64; 25] = [0u64; 25usize];
    ((&mut tmp)[0usize..25usize]).copy_from_slice(&tmp_block_state[0usize..25usize]);
    crate::hacl::mac_poly1305::poly1305_finish(output, k·, &mut tmp)
}

pub fn mac(output: &mut [u8], input: &[u8], input_len: u32, key: &[u8]) {
    let mut ctx: [u64; 25] = [0u64; 25usize];
    crate::hacl::mac_poly1305::poly1305_init(&mut ctx, key);
    crate::hacl::mac_poly1305::poly1305_update(&mut ctx, input_len, input);
    crate::hacl::mac_poly1305::poly1305_finish(output, key, &mut ctx)
}
