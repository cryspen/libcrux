...
fn build_x25519(out_path: &Path) {
    let mut files = vec!["x25519_ref.s", "x25519_mulx.s"];
    ...
    build_cmd
        ...
        .args(&[
            "-r",
            &out_path.join("libx25519.a").display().to_string(),
        ])
        .args(&object_files);
    println!(" >>> build_cmd: {:?}", build_cmd);
    println!("     current dir: {:?}", build_cmd.get_current_dir());
...
fn main() {
    ...
    build_x25519(out_path);

    // Set library name to look up
    let library_name = "x25519";
