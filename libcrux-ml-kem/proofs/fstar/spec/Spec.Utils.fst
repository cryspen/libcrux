...
let v_G input = map_slice (fun x -> from_uint8 (Lib.RawIntTypes.u8_to_UInt8 x)) (sha3_512 (Seq.length input) (map_slice (fun x -> Lib.IntTypes.secret #Lib.IntTypes.U8 (to_uint8 x)) input))

val v_H (input: t_Slice u8) : t_Array u8 (sz 32)
let v_H input = map_slice (fun x -> from_uint8 (Lib.RawIntTypes.u8_to_UInt8 x))  (sha3_256 (Seq.length input) (map_slice (fun x -> Lib.IntTypes.secret #Lib.IntTypes.U8 (to_uint8 x)) input))

val v_PRF (v_LEN: usize{v v_LEN < pow2 32}) (input: t_Slice u8) : t_Array u8 v_LEN
let v_PRF v_LEN input = map_slice 
  (fun x -> from_uint8 (Lib.RawIntTypes.u8_to_UInt8 x))   
  (shake256 (Seq.length input) 
...
let v_XOF v_LEN input = map_slice   (fun x -> from_uint8 (Lib.RawIntTypes.u8_to_UInt8 x))   

(
  shake128 (Seq.length input)           (map_slice (fun x -> Lib.IntTypes.secret #Lib.IntTypes.U8 (to_uint8 x)) input) 
  (v v_LEN))
