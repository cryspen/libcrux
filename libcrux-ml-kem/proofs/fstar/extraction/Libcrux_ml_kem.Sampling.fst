...
let sample_from_uniform_distribution_next
      ...
      (out: t_Array (t_Array i16 (sz 256)) v_K)
     =
  let out, sampled_coefficients:(t_Array (t_Array i16 (sz 256)) v_K & t_Array usize v_K) =
    Core.Iter.Traits.Iterator.f_fold (Core.Iter.Traits.Collect.f_into_iter ({
        ...
        <:
        Core.Ops.Range.t_Range usize)
      (out, sampled_coefficients <: (t_Array (t_Array i16 (sz 256)) v_K & t_Array usize v_K))
      (fun temp_0_ i ->
          let out, sampled_coefficients:(t_Array (t_Array i16 (sz 256)) v_K & t_Array usize v_K) =
            temp_0_
          in
          ...
          Core.Iter.Traits.Iterator.f_fold (Core.Iter.Traits.Collect.f_into_iter ({
            ...
            (out, sampled_coefficients <: (t_Array (t_Array i16 (sz 256)) v_K & t_Array usize v_K))
            (fun temp_0_ r ->
                let out, sampled_coefficients:(t_Array (t_Array i16 (sz 256)) v_K &
                  t_Array usize v_K) =
                  temp_0_
                in
                let r:usize = r in
                ...
                if remaining >. sz 0
                then
                  let sampled, vec:(usize & t_Array i16 (sz 16)) =
                    Libcrux_traits.f_rej_sample ((randomness.[ i ] <: t_Array u8 v_N).[ {
                        ...
                        t_Slice u8)
                  in
                  let pick:usize = if sampled <=. remaining then sampled else remaining in
                  let out:t_Array (t_Array i16 (sz 256)) v_K =
                    Rust_primitives.Hax.Monomorphized_update_at.update_at_usize out
                      ...
                      (Rust_primitives.Hax.Monomorphized_update_at.update_at_range (out.[ i ]
                          ...
                          ({
                              ...
                              =
                              (sampled_coefficients.[ i ] <: usize) +! pick <: usize
                            }
                            <:
                            Core.Ops.Range.t_Range usize)
                          (Core.Slice.impl__copy_from_slice ((out.[ i ] <: t_Array i16 (sz 256)).[ {
                                    Core.Ops.Range.f_start = sampled_coefficients.[ i ] <: usize;
                                    Core.Ops.Range.f_end
                                    =
                                    (sampled_coefficients.[ i ] <: usize) +! pick <: usize
                                ...
                                <:
                                t_Slice i16)
                              (vec.[ { Core.Ops.Range.f_start = sz 0; Core.Ops.Range.f_end = pick }
                                  <:
                                  Core.Ops.Range.t_Range usize ]
                                <:
                                t_Slice i16)
                  ...
                  <:
                  (t_Array (t_Array i16 (sz 256)) v_K & t_Array usize v_K)
                else
                  out, sampled_coefficients
                  <:
                  (t_Array (t_Array i16 (sz 256)) v_K & t_Array usize v_K))
  ...
  <:
  (t_Array usize v_K & t_Array (t_Array i16 (sz 256)) v_K & bool)
      ...
      (fun sampled_i16s temp_1_ ->
          ...
          Core.Iter.Traits.Iterator.f_fold (Core.Iter.Traits.Collect.f_into_iter (Core.Iter.Traits.Iterator.f_step_by
            ...
            (fun sampled_i16s outcome_set ->
                ...
                sampled_i16s))
  in
  Libcrux_ml_kem.Polynomial.impl__from_i16_array sampled_i16s
...
let sample_from_xof
  ...
  in
  let (out: t_Array (t_Array i16 (sz 256)) v_K):t_Array (t_Array i16 (sz 256)) v_K =
    Rust_primitives.Hax.repeat (Rust_primitives.Hax.repeat 0s (sz 256) <: t_Array i16 (sz 256)) v_K
  in
  ...
  let tmp0, tmp1, out1:(t_Array usize v_K & t_Array (t_Array i16 (sz 256)) v_K & bool) =
  ...
  let out:t_Array (t_Array i16 (sz 256)) v_K = tmp1 in
  let done:bool = out1 in
  let done, out, sampled_coefficients, xof_state:(bool & t_Array (t_Array i16 (sz 256)) v_K &
    t_Array usize v_K &
    ...
    Rust_primitives.f_while_loop (fun temp_0_ ->
      ...
      (done, out, sampled_coefficients, xof_state
        <:
        (bool & t_Array (t_Array i16 (sz 256)) v_K & t_Array usize v_K &
          Libcrux_sha3.X4.t_Shake128StateX4))
      (fun temp_0_ ->
          let done, out, sampled_coefficients, xof_state:(bool & t_Array (t_Array i16 (sz 256)) v_K &
            t_Array usize v_K &
  ...
  Core.Array.impl_23__map v_K out Libcrux_ml_kem.Polynomial.impl__from_i16_array
