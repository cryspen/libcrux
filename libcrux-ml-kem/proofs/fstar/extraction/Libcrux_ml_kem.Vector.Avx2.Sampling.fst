...
let rejection_sample (input: t_Slice u8) (output: t_Slice i16) =
  ...
  let _:Prims.unit =
    ...
    assume (v (cast (Core.Num.impl_u8__count_ones good.[ sz 0 ]) <: usize) <= 8);
    assume (v (cast (Core.Num.impl_u8__count_ones good.[ sz 1 ]) <: usize) <= 8);
    assume (Core.Ops.Index.f_index_pre output
          ({
              Core.Ops.Range.f_start = cast (Core.Num.impl_u8__count_ones good.[ sz 0 ]) <: usize;
              Core.Ops.Range.f_end
              =
              (cast (Core.Num.impl_u8__count_ones good.[ sz 0 ]) <: usize) +! sz 8
            }))
  in
  ...
  let sampled_count:usize =
    cast (Core.Num.impl_u8__count_ones (good.[ mk_usize 0 ] <: u8) <: u32) <: usize
  in
