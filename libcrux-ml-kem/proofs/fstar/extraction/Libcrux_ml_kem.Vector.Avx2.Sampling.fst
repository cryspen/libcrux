...
let rejection_sample (input: t_Slice u8) (output: t_Slice i16) =
  ...
  in
  let potential_coefficients:Core.Core_arch.X86.t____m256i =
    Libcrux_ml_kem.Vector.Avx2.Serialize.deserialize_12_ input
  in
  let compare_with_field_modulus:Core.Core_arch.X86.t____m256i =
    Libcrux_intrinsics.Avx2.mm256_cmpgt_epi16 field_modulus potential_coefficients
  in
  ...
  let lower_shuffles:t_Array u8 (sz 16) =
    Libcrux_ml_kem.Vector.Rej_sample_table.v_REJECTION_SAMPLE_SHUFFLE_TABLE.[ cast (good.[ sz 0 ]
      ...
      usize ]
  in
  let lower_shuffles:Core.Core_arch.X86.t____m128i =
    Libcrux_intrinsics.Avx2.mm_loadu_si128 (Rust_primitives.unsize lower_shuffles <: t_Slice u8)
  in
  ...
  in
  let output:t_Slice i16 = Libcrux_intrinsics.Avx2.mm_storeu_si128 output lower_coefficients in
  ...
  let upper_shuffles:t_Array u8 (sz 16) =
    Libcrux_ml_kem.Vector.Rej_sample_table.v_REJECTION_SAMPLE_SHUFFLE_TABLE.[ cast (good.[ sz 1 ]
      ...
      usize ]
  in
  let upper_shuffles:Core.Core_arch.X86.t____m128i =
    Libcrux_intrinsics.Avx2.mm_loadu_si128 (Rust_primitives.unsize upper_shuffles <: t_Slice u8)
  in
  let upper_coefficients:Core.Core_arch.X86.t____m128i =
    Libcrux_intrinsics.Avx2.mm256_extracti128_si256 1l potential_coefficients
  in
  let upper_coefficients:Core.Core_arch.X86.t____m128i =
  ...
  let output:t_Slice i16 =
    Rust_primitives.Hax.Monomorphized_update_at.update_at_range output
      ...
      (Libcrux_intrinsics.Avx2.mm_storeu_si128 (output.[ {
                Core.Ops.Range.f_start = sampled_count;
                Core.Ops.Range.f_end = sampled_count +! sz 8 <: usize
              }
              <:
