...
let rejection_sample (input: t_Slice u8) (output: t_Slice i16) =
  ...
  in
  let good:t_Array u8 (Rust_primitives.mk_usize 2) =
  ...
  let lower_shuffles:t_Array u8 (Rust_primitives.mk_usize 16) =
    Libcrux_ml_kem.Vector.Rej_sample_table.v_REJECTION_SAMPLE_SHUFFLE_TABLE.[ cast (good.[ Rust_primitives.mk_usize
            0 ]
          <:
          u8)
      <:
  ...
  let sampled_count:usize =
    cast (Core.Num.impl__u8__count_ones (good.[ Rust_primitives.mk_usize 0 ] <: u8) <: u32) <: usize
  in
  ...
  let output:t_Slice i16 =
    Rust_primitives.Hax.Monomorphized_update_at.update_at_range output
      ({
          ...
          Core.Ops.Range.f_end = sampled_count +! Rust_primitives.mk_usize 8 <: usize
        }
        <:
  ...
  let hax_temp_output:usize =
    sampled_count +!
    (cast (Core.Num.impl__u8__count_ones (good.[ Rust_primitives.mk_usize 1 ] <: u8) <: u32)
      <:
      usize)
  in
