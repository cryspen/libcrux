...
let free_state (xof_state: Libcrux_sha3.X4.t_Shake128StateX4) =
  let _:Prims.unit = Libcrux_sha3.X4.impl__Shake128StateX4__free_memory xof_state in
  ()

let v_H (input: t_Slice u8) = Libcrux_sha3.sha256 input

let v_G (input: t_Slice u8) = Libcrux_sha3.sha512 input

let v_PRF (v_LEN: usize) (input: t_Slice u8) = Libcrux_sha3.shake256 v_LEN input

let squeeze_block (v_K: usize) (xof_state: Libcrux_sha3.X4.t_Shake128StateX4) =
  let tmp0, out1:(Libcrux_sha3.X4.t_Shake128StateX4 & t_Array (t_Array u8 (sz 168)) v_K) =
    Libcrux_sha3.X4.impl__Shake128StateX4__squeeze_blocks (sz 168) v_K xof_state
  in
  ...
  <:
  (Libcrux_sha3.X4.t_Shake128StateX4 & t_Array (t_Array u8 (sz 168)) v_K)

let squeeze_three_blocks (v_K: usize) (xof_state: Libcrux_sha3.X4.t_Shake128StateX4) =
  let tmp0, out1:(Libcrux_sha3.X4.t_Shake128StateX4 & t_Array (t_Array u8 (sz 504)) v_K) =
    Libcrux_sha3.X4.impl__Shake128StateX4__squeeze_blocks (sz 504) v_K xof_state
  in
  let xof_state:Libcrux_sha3.X4.t_Shake128StateX4 = tmp0 in
  ...
  xof_state, hax_temp_output
  <:
  (Libcrux_sha3.X4.t_Shake128StateX4 & t_Array (t_Array u8 (sz 504)) v_K)
...
let absorb (v_K: usize) (input: t_Array (t_Array u8 (sz 34)) v_K) =
  let _:Prims.unit =
    ...
    then
      ...
      ()
  in
  let state:Libcrux_sha3.X4.t_Shake128StateX4 = Libcrux_sha3.X4.impl__Shake128StateX4__new () in
  ...
  in
  let state:Libcrux_sha3.X4.t_Shake128StateX4 =
    Libcrux_sha3.X4.impl__Shake128StateX4__absorb_final v_K state data
  in
  state
