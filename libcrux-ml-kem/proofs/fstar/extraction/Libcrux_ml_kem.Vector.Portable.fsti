...
Libcrux_ml_kem.Vector.Portable.Vector_type.t_PortableVector =
  {
    ...
    f_rej_sample_pre
    =
    (fun (a: t_Slice u8) (output: t_Slice i16) -> (Core.Slice.impl__len #u8 a <: usize) =. sz 24);
    f_rej_sample_post
    =
    (fun (a: t_Slice u8) (output: t_Slice i16) (output_future, result: (t_Slice i16 & usize)) ->
        Seq.length output_future == Seq.length output);
    f_rej_sample
    =
    fun (a: t_Slice u8) (output: t_Slice i16) ->
      let tmp0, out:(t_Slice i16 & usize) =
        Libcrux_ml_kem.Vector.Portable.Sampling.rej_sample a output
      in
