...
let sample_ring_element_cbd
  ...
  let _:Prims.unit =
    assume (Libcrux_ml_kem.Hash_functions.f_PRFxN_pre #v_Hasher
          #v_K
          #(FStar.Tactics.Effect.synth_by_tactic (fun _ -> (FStar.Tactics.Typeclasses.tcresolve ()))
          )
...
let encrypt
  ...
  in
  let _:Prims.unit =
    Hax_lib.v_assume (((Rust_primitives.Hax.Int.from_machine domain_separator <: Hax_lib.Int.t_Int) +
  ...
  let error_1_, domain_separator:(t_Array
    ...
    sample_ring_element_cbd #v_K
      ...
      domain_separator
  in
  let _:Prims.unit = assert (v (sz 32) < Seq.length prf_input) in
  ...
  let u:t_Array (Libcrux_ml_kem.Polynomial.t_PolynomialRingElement v_Vector) v_K =
    Libcrux_ml_kem.Matrix.compute_vector_u #v_K #v_Vector v_A_transpose r_as_ntt error_1_
  in
  ...
  let _:Prims.unit =
    assume (Seq.length (compress_then_serialize_u #v_K
              #v_C1_LEN
              #v_U_COMPRESSION_FACTOR
              #v_BLOCK_LEN
              #v_Vector
            ...
            <:
            t_Slice u8) ==
        Seq.length (ciphertext.[ { Core.Ops.Range.f_start = sz 0; Core.Ops.Range.f_end = v_C1_LEN }
              <:
              Core.Ops.Range.t_Range usize ]
            <:
            t_Slice u8))
  ...
  in
  let _:Prims.unit =
    assume (Seq.length (Libcrux_ml_kem.Serialize.compress_then_serialize_ring_element_v #v_V_COMPRESSION_FACTOR
              #v_C2_LEN
              #v_Vector
              v
              (ciphertext.[ { Core.Ops.Range.f_start = v_C1_LEN }
                  <:
                  Core.Ops.Range.t_RangeFrom usize ]
                <:
                t_Slice u8)
            <:
            t_Slice u8) ==
        Seq.length (ciphertext.[ { Core.Ops.Range.f_start = v_C1_LEN }
              <:
              Core.Ops.Range.t_RangeFrom usize ]
            <:
            t_Slice u8))
...
let decrypt
  ...
  let message:Libcrux_ml_kem.Polynomial.t_PolynomialRingElement v_Vector =
    Libcrux_ml_kem.Matrix.compute_message #v_K #v_Vector v secret_as_ntt u_as_ntt
  in
...
let serialize_public_key
     ...
     =
  let _:Prims.unit =
    assert (v v_PUBLIC_KEY_SIZE - v v_RANKED_BYTES_PER_RING_ELEMENT ==
...
let generate_keypair
     ...
     =
  let _:Prims.unit =
    assume (Libcrux_ml_kem.Hash_functions.f_G_pre #v_Hasher #v_K key_generation_seed)
  in
  ...
  in
  let _:Prims.unit =
    Hax_lib.v_assume ((Rust_primitives.Hax.Int.from_machine v_K <: Hax_lib.Int.t_Int) <=
        (Rust_primitives.Hax.Int.from_machine 255l <: Hax_lib.Int.t_Int)
        <:
        bool)
  in
  ...
  in
  let _:Prims.unit =
    Hax_lib.v_assume ((v_RANKED_BYTES_PER_RING_ELEMENT <=. v_PUBLIC_KEY_SIZE <: bool) &&
  ...
  let secret_key_serialized:t_Array u8 v_PRIVATE_KEY_SIZE =
    serialize_secret_key #v_K #v_PRIVATE_KEY_SIZE #v_Vector secret_as_ntt
  in
