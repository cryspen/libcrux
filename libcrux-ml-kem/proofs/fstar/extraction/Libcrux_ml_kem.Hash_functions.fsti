...
class t_Hash (v_Self: Type0) (v_K: usize) = {
  ...
  f_shake128_init_absorb_final_pre:input: t_Array (t_Array u8 (Rust_primitives.mk_usize 34)) v_K
    -> pred: Type0{true ==> pred};
  f_shake128_init_absorb_final_post:t_Array (t_Array u8 (Rust_primitives.mk_usize 34)) v_K -> v_Self
    -> Type0;
  f_shake128_init_absorb_final:x0: t_Array (t_Array u8 (Rust_primitives.mk_usize 34)) v_K
    -> Prims.Pure v_Self
  ...
  f_shake128_squeeze_first_three_blocks_post:
      v_Self ->
      (v_Self & t_Array (t_Array u8 (Rust_primitives.mk_usize 504)) v_K)
    -> Type0;
  f_shake128_squeeze_first_three_blocks:x0: v_Self
    -> Prims.Pure (v_Self & t_Array (t_Array u8 (Rust_primitives.mk_usize 504)) v_K)
  ...
  f_shake128_squeeze_next_block_post:
      v_Self ->
      (v_Self & t_Array (t_Array u8 (Rust_primitives.mk_usize 168)) v_K)
    -> Type0;
  f_shake128_squeeze_next_block:x0: v_Self
    -> Prims.Pure (v_Self & t_Array (t_Array u8 (Rust_primitives.mk_usize 168)) v_K)
        (f_shake128_squeeze_next_block_pre x0)
        (fun result -> f_shake128_squeeze_next_block_post x0 result)
}

/// The SHA3 block size.
let v_BLOCK_SIZE: usize = Rust_primitives.mk_usize 168

/// The size of 3 SHA3 blocks.
let v_THREE_BLOCKS: usize = v_BLOCK_SIZE *! Rust_primitives.mk_usize 3
