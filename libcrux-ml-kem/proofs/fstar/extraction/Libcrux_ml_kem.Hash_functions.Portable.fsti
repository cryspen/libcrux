...
val shake128_squeeze_first_three_blocks (v_K: usize) (st: t_PortableHash v_K)
    : Prims.Pure (t_PortableHash v_K & t_Array (t_Array u8 (Rust_primitives.mk_usize 504)) v_K)
      Prims.l_True
      (fun _ -> Prims.l_True)

val shake128_squeeze_next_block (v_K: usize) (st: t_PortableHash v_K)
    : Prims.Pure (t_PortableHash v_K & t_Array (t_Array u8 (Rust_primitives.mk_usize 168)) v_K)
      Prims.l_True
      (fun _ -> Prims.l_True)
...
let impl (v_K: usize) : Libcrux_ml_kem.Hash_functions.t_Hash (t_PortableHash v_K) v_K =
  {
    ...
    (fun
        ...
        (out1: (t_PortableHash v_K & t_Array (t_Array u8 (Rust_primitives.mk_usize 504)) v_K))
        ->
        true);
    f_shake128_squeeze_first_three_blocks
    =
    (fun (self: t_PortableHash v_K) ->
        let tmp0, out:(t_PortableHash v_K & t_Array (t_Array u8 (Rust_primitives.mk_usize 504)) v_K)
        =
        ...
        <:
        (t_PortableHash v_K & t_Array (t_Array u8 (Rust_primitives.mk_usize 504)) v_K));
    f_shake128_squeeze_next_block_pre = (fun (self: t_PortableHash v_K) -> true);
    f_shake128_squeeze_next_block_post
    =
    (fun
        (self: t_PortableHash v_K)
        (out1: (t_PortableHash v_K & t_Array (t_Array u8 (Rust_primitives.mk_usize 168)) v_K))
        ->
        true);
    f_shake128_squeeze_next_block
    =
    fun (self: t_PortableHash v_K) ->
      let tmp0, out:(t_PortableHash v_K & t_Array (t_Array u8 (Rust_primitives.mk_usize 168)) v_K) =
      ...
      self, hax_temp_output
      <:
      (t_PortableHash v_K & t_Array (t_Array u8 (Rust_primitives.mk_usize 168)) v_K)
  }
