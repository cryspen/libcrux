...
type t_PortableHash (v_K: usize) = {
  f_shake128_state:t_Array (Libcrux_sha3.Generic_keccak.t_KeccakState (sz 1) u64) v_K
}
          ...
          Core.Iter.Traits.Iterator.f_fold (Core.Iter.Traits.Collect.f_into_iter ({
            ...
            (fun out i ->
                ...
                Rust_primitives.Hax.Monomorphized_update_at.update_at_usize out
                  i
                  (Libcrux_sha3.Portable.shake256 v_LEN
                      (out.[ i ] <: t_Array u8 v_LEN)
    ...
    (fun (input: t_Array (t_Array u8 (sz 34)) v_K) ->
        ...
        let state:t_Array (Libcrux_sha3.Generic_keccak.t_KeccakState (sz 1) u64) v_K =
          Rust_primitives.Hax.repeat (Libcrux_sha3.Portable.Incremental.shake128_init ()
              <:
              Libcrux_sha3.Generic_keccak.t_KeccakState (sz 1) u64)
            v_K
        in
        let state:t_Array (Libcrux_sha3.Generic_keccak.t_KeccakState (sz 1) u64) v_K =
          Core.Iter.Traits.Iterator.f_fold (Core.Iter.Traits.Collect.f_into_iter ({
            ...
            (fun state i ->
                ...
                Rust_primitives.Hax.Monomorphized_update_at.update_at_usize state
                  ...
                  (Libcrux_sha3.Portable.Incremental.shake128_absorb_final (state.[ i ]
                    ...
                    Libcrux_sha3.Generic_keccak.t_KeccakState (sz 1) u64)
                <:
                t_Array (Libcrux_sha3.Generic_keccak.t_KeccakState (sz 1) u64) v_K)
        in
