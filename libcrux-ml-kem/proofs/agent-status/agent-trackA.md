...
## 2026-04-29 evening — USER-13 lane (`invert_ntt_at_layer_2` body)

Worktree `~/libcrux-ml-kem-lane-A5/`, branch `agent/lane-A5` rebased
onto `trait-opacify` HEAD `8bf91ca56`.  Session goal: close the
`--admit_smt_queries true` on `invert_ntt_at_layer_2` filed at
Wave-B Snapshot 3.

### Wall diagnosed (NOT what the prompt predicted)

Removed the admit + ran with `--query_stats --split_queries always
--z3rlimit 200` to localise the saturation.  Result: **Q108 cancels
at 42.4 s** at the iter-end loop-invariant subtyping check (line
195's `acc'` typing).  The saturation is purely on the bound
conjunct's transit from the trait post into the next-iter
invariant — NOT on the FE-algebra `forall4 inv_ntt_layer_2_step_branch_post`
conjunct (those are already opaque atoms).

The pre-existing `reveal_opaque (\`%is_i16b_array_opaque)
(is_i16b_array_opaque)` inside the loop body unfolds the OPAQUE
bound predicate into a 16-lane `forall i. -n <= a.[i] /\ a.[i] <= n`.
Each iteration re-injects this `forall` for every previously-bounded
chunk, polluting Z3's context with dozens of quantifier triggers.
This is the wall.

### Fix (single-shot)

Removed the global `reveal_opaque` and added a single targeted
post-call assert:
...
This injects the per-chunk bound atom directly in scope (matches
the trait post's first conjunct via opacity, no unfolding) and lets
Z3 carry just that one atom into the loop invariant subtyping.
Final pragma: `--z3rlimit 100` (no `--ext context_pruning`, no
`--split_queries always` needed — those were the prior session's
escape hatches).

**SD3 opaque-wrapper not added.**  The `Hacspec_ml_kem.Commute.Chunk.fst`
file is unchanged.  The proven SD3 pattern from `b7b49c358` (4
per-branch + per-lane wrapper + per-vector bridge) is consumed by
`Bridges.fst::lemma_inv_ntt_layer_2_step_lane_bridge` — that
machinery handles the FE-algebra side of the trait post and was
already in place.  The wall was purely on the impl-side body's
bounds, not on the bridge.

### Closure stats

| Metric | Wave-B (admit) | This session (closed) |
|---|---|---|
| Pragma | `--admit_smt_queries true` | `--z3rlimit 100` |
| Total wall on `invert_ntt_at_layer_2_` | n/a | 2.1 s |
| Max single query | n/a | 2028 ms |
| Queries | 0 | 2 |
| Failed / saturated | 0 / 0 | 0 / 0 |

`make check/Libcrux_ml_kem.Invert_ntt.fst` cold (with hint record):
**126.4 s** (vs 153 s+ in admitted state when the slow Bridges
lemma was actively rebuilt — the savings come from not retrying
the Q108 wall).  Replay: `7 s`.

Full-tree make `EXIT=0` post-merge, ~659 s total wall.  Single
out-of-band issue: the hax codegen bug (duplicate `noeq` on
`Vector.Neon.Vector_type.fsti`) re-emerged after `python3 hax.py
extract` and was masked locally per the standing workaround
(per `feedback_drive_to_top_spike` + Phase 6d note in
`fstar-perf-top20.md`).

### `lemma_inv_ntt_layer_2_step_lane_bridge` perf — unchanged

Bridges.fst was not edited.  Snapshot 3's 146.2 s post-hint-record
wall on this lemma still holds — unrelated to the body fix.  Filed
as a separate concern: this is a persistent quantifier wall on the
SPEC-side, not on the impl-side admit chain.  Recommendation for
follow-up: reproduce with `smtprofiling` + try the `--smtencoding.elim_box`
escape hatch.

### Hot files touched

  - `libcrux-ml-kem/src/invert_ntt.rs` — `invert_ntt_at_layer_2`
    options + body F* fragment (assert added, global reveal removed).
  - `libcrux-ml-kem/proofs/agent-status/fstar-perf-top20.md`
    (Snapshot 4 appended).
  - `libcrux-ml-kem/proofs/fstar/extraction/Libcrux_ml_kem.Vector.Neon.Vector_type.fsti`
    (local sed-fix for duplicate `noeq` — known re-extraction
    artifact; see Phase 6d note).
  - `libcrux-ml-kem/proofs/fstar/extraction/Libcrux_ml_kem.Invert_ntt.fst`
    (regenerated by hax extract).
  - Other auto-regen .fst/.fsti from extraction (Vector.{Avx2,Portable},
    Vector.Traits, Serialize) — diff is auto-extraction drift not
    caused by this change set.

### Net delta

  - **Admit delta**: -1 (USER-13 admit removed).  3 portable+AVX2
    admits remain (USER-14: 2× from_bytes/to_bytes + layer_4_plus
    body; USER-15: invert_ntt_montgomery body).
  - **lax/panic_free delta**: 0.
  - **Critical path advance**: ✅ — Wave-B's top-1 perf culprit
    (admitted body) is now closed at 2.1 s; the remaining
    Wave-B-filed culprits (USER-14, USER-15) are now the
    sole gating items between A5's strengthened post chain and
    the Wave-C kickoff (Matrix → Ind_cpa → Ind_cca).
