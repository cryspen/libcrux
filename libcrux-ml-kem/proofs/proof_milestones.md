...
|---|---|---|---|---|
| 1 | Forward NTT layer 1-7 correct vs hacspec | `src/ntt.rs::ntt_at_layer_{1,2,3,4_plus,7}` | ⚠️ partial — layer 1 ✅ (commit `c32653051`, mirror of inverse-layer-1 pattern, verified 349 s); layers 2/3/4_plus/7 ❌ no claim. Per-vector bridges `lemma_ntt_layer_{2,3}_step_to_hacspec` don't exist yet (only chunk-level commutes in Chunk.fst); writing them is a spec-side authoring task (~1 session each, mirror today's inverse-layer-2 bridge in `4672cc005`). Layer 7 is structurally different (single zeta, between-chunk butterfly) — needs its own bridge design. | ~2-3 sprints — author per-layer Bridges lemmas (2/3/7), then per-impl hacspec citation each ~1 session at z3rlimit 800. |
