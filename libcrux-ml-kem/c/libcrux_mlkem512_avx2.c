...
- IMPLICIT_REJECTION_HASH_INPUT_SIZE= 800
*/
static void decapsulate_unpacked_36(
...
void libcrux_ml_kem_mlkem512_avx2_decapsulate_unpacked(
    ...
    libcrux_ml_kem_types_MlKemCiphertext_e8 *ciphertext, uint8_t ret[32U]) {
  decapsulate_unpacked_36(private_key, ciphertext, ret);
}
...
- ETA2_RANDOMNESS_SIZE= 128
*/
static tuple_ec encapsulate_93(
...
tuple_ec libcrux_ml_kem_mlkem512_avx2_encapsulate(
  ...
  uint8_t copy_of_randomness[32U];
  memcpy(copy_of_randomness, randomness, (size_t)32U * sizeof(uint8_t));
  return encapsulate_93(uu____0, copy_of_randomness);
}
...
- ETA2_RANDOMNESS_SIZE= 128
*/
static tuple_ec encapsulate_unpacked_ff(
...
- PUBLIC_KEY_SIZE= 800
*/
static bool validate_public_key_ea(uint8_t *public_key) {
...
core_option_Option_04 libcrux_ml_kem_mlkem512_avx2_validate_public_key(
  ...
  if (validate_public_key_ea(public_key.value)) {
