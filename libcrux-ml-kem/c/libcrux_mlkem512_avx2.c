...
*/
static void decapsulate_0b(libcrux_ml_kem_types_MlKemPrivateKey_5e *private_key,
                           libcrux_ml_kem_types_MlKemCiphertext_e8 *ciphertext,
                           uint8_t ret[32U]) {
  libcrux_ml_kem_ind_cca_decapsulate_dd(private_key, ciphertext, ret);
}
...
- IMPLICIT_REJECTION_HASH_INPUT_SIZE= 800
*/
static void decapsulate_unpacked_09(
...
void libcrux_ml_kem_mlkem512_avx2_decapsulate_unpacked(
    ...
    libcrux_ml_kem_types_MlKemCiphertext_e8 *ciphertext, uint8_t ret[32U]) {
  decapsulate_unpacked_09(private_key, ciphertext, ret);
}
...
- ETA2_RANDOMNESS_SIZE= 128
*/
static tuple_ec encapsulate_4a(
...
tuple_ec libcrux_ml_kem_mlkem512_avx2_encapsulate(
  ...
  uint8_t copy_of_randomness[32U];
  memcpy(copy_of_randomness, randomness, (size_t)32U * sizeof(uint8_t));
  return encapsulate_4a(uu____0, copy_of_randomness);
}
...
- ETA2_RANDOMNESS_SIZE= 128
*/
static tuple_ec encapsulate_unpacked_64(
  ...
  uint8_t copy_of_randomness[32U];
  memcpy(copy_of_randomness, randomness, (size_t)32U * sizeof(uint8_t));
  return libcrux_ml_kem_ind_cca_unpacked_encapsulate_unpacked_fd(
...
tuple_ec libcrux_ml_kem_mlkem512_avx2_encapsulate_unpacked(
  ...
  uint8_t copy_of_randomness[32U];
  memcpy(copy_of_randomness, randomness, (size_t)32U * sizeof(uint8_t));
  return encapsulate_unpacked_64(uu____0, copy_of_randomness);
}
...
- ETA1_RANDOMNESS_SIZE= 192
*/
static libcrux_ml_kem_types_MlKemKeyPair_cb generate_keypair_7c(
...
libcrux_ml_kem_mlkem512_avx2_generate_key_pair(uint8_t randomness[64U]) {
  ...
  uint8_t copy_of_randomness[64U];
  memcpy(copy_of_randomness, randomness, (size_t)64U * sizeof(uint8_t));
  return generate_keypair_7c(copy_of_randomness);
}
...
generate_keypair_unpacked_cc(uint8_t randomness[64U]) {
  /* Passing arrays by value in Rust generates a copy in C */
  uint8_t copy_of_randomness[64U];
  memcpy(copy_of_randomness, randomness, (size_t)64U * sizeof(uint8_t));
...
libcrux_ml_kem_mlkem512_avx2_generate_key_pair_unpacked(
  ...
  uint8_t copy_of_randomness[64U];
  memcpy(copy_of_randomness, randomness, (size_t)64U * sizeof(uint8_t));
  return generate_keypair_unpacked_cc(copy_of_randomness);
}
...
- PUBLIC_KEY_SIZE= 800
*/
static bool validate_public_key_f1(uint8_t *public_key) {
...
core_option_Option_04 libcrux_ml_kem_mlkem512_avx2_validate_public_key(
  ...
  if (validate_public_key_f1(public_key.value)) {
