...
void libcrux_ml_kem_mlkem768_avx2_decapsulate(
    libcrux_ml_kem_types_MlKemPrivateKey_55 *private_key,
    libcrux_ml_kem_mlkem768_MlKem768Ciphertext *ciphertext, uint8_t ret[32U]) {
  decapsulate_ee1(private_key, ciphertext, ret);
}
...
- ETA2_RANDOMNESS_SIZE= 128
*/
static tuple_3c encapsulate_371(
  ...
  return libcrux_ml_kem_ind_cca_encapsulate_c21(uu____0, copy_of_randomness);
}
...
tuple_3c libcrux_ml_kem_mlkem768_avx2_encapsulate(
  ...
  uint8_t copy_of_randomness[32U];
  memcpy(copy_of_randomness, randomness, (size_t)32U * sizeof(uint8_t));
  return encapsulate_371(uu____0, copy_of_randomness);
}
...
- ETA1_RANDOMNESS_SIZE= 128
*/
static libcrux_ml_kem_mlkem768_MlKem768KeyPair generate_keypair_201(
...
libcrux_ml_kem_mlkem768_avx2_generate_key_pair(uint8_t randomness[64U]) {
  ...
  uint8_t copy_of_randomness[64U];
  memcpy(copy_of_randomness, randomness, (size_t)64U * sizeof(uint8_t));
  return generate_keypair_201(copy_of_randomness);
}
...
- CIPHERTEXT_SIZE= 1088
*/
static KRML_MUSTINLINE bool validate_private_key_5f1(
    libcrux_ml_kem_types_MlKemPrivateKey_55 *private_key,
    libcrux_ml_kem_mlkem768_MlKem768Ciphertext *ciphertext) {
  return libcrux_ml_kem_ind_cca_validate_private_key_c61(private_key,
                                                         ciphertext);
}
...
bool libcrux_ml_kem_mlkem768_avx2_validate_private_key(
    libcrux_ml_kem_types_MlKemPrivateKey_55 *private_key,
    libcrux_ml_kem_mlkem768_MlKem768Ciphertext *ciphertext) {
  return validate_private_key_5f1(private_key, ciphertext);
}
...
- PUBLIC_KEY_SIZE= 1184
*/
static KRML_MUSTINLINE bool validate_public_key_4a1(uint8_t *public_key) {
  return libcrux_ml_kem_ind_cca_validate_public_key_2e1(public_key);
}
...
bool libcrux_ml_kem_mlkem768_avx2_validate_public_key(
    libcrux_ml_kem_types_MlKemPublicKey_15 *public_key) {
  return validate_public_key_4a1(public_key->value);
}
