...
void libcrux_ml_kem_mlkem1024_avx2_decapsulate(
    libcrux_ml_kem_types_MlKemPrivateKey_95 *private_key,
    libcrux_ml_kem_types_MlKemCiphertext_1f *ciphertext, uint8_t ret[32U]) {
  decapsulate_d60(private_key, ciphertext, ret);
}
...
- ETA2_RANDOMNESS_SIZE= 128
*/
static tuple_21 encapsulate_200(
  ...
  return libcrux_ml_kem_ind_cca_encapsulate_4a0(uu____0, copy_of_randomness);
}
...
tuple_21 libcrux_ml_kem_mlkem1024_avx2_encapsulate(
  ...
  uint8_t copy_of_randomness[32U];
  memcpy(copy_of_randomness, randomness, (size_t)32U * sizeof(uint8_t));
  return encapsulate_200(uu____0, copy_of_randomness);
}
...
- ETA1_RANDOMNESS_SIZE= 128
*/
static libcrux_ml_kem_mlkem1024_MlKem1024KeyPair generate_keypair_700(
...
libcrux_ml_kem_mlkem1024_avx2_generate_key_pair(uint8_t randomness[64U]) {
  ...
  uint8_t copy_of_randomness[64U];
  memcpy(copy_of_randomness, randomness, (size_t)64U * sizeof(uint8_t));
  return generate_keypair_700(copy_of_randomness);
}
...
- CIPHERTEXT_SIZE= 1568
*/
static KRML_MUSTINLINE bool validate_private_key_290(
    libcrux_ml_kem_types_MlKemPrivateKey_95 *private_key,
...
bool libcrux_ml_kem_mlkem1024_avx2_validate_private_key(
    libcrux_ml_kem_types_MlKemPrivateKey_95 *private_key,
    libcrux_ml_kem_types_MlKemCiphertext_1f *ciphertext) {
  return validate_private_key_290(private_key, ciphertext);
}
...
- PUBLIC_KEY_SIZE= 1568
*/
static KRML_MUSTINLINE bool validate_public_key_e70(uint8_t *public_key) {
  return libcrux_ml_kem_ind_cca_validate_public_key_330(public_key);
}
...
bool libcrux_ml_kem_mlkem1024_avx2_validate_public_key(
    libcrux_ml_kem_types_MlKemPublicKey_1f *public_key) {
  return validate_public_key_e70(public_key->value);
}
