...
void libcrux_ml_kem_mlkem768_portable_decapsulate(
    libcrux_ml_kem_types_MlKemPrivateKey_55 *private_key,
    libcrux_ml_kem_mlkem768_MlKem768Ciphertext *ciphertext, uint8_t ret[32U]) {
  decapsulate_6b(private_key, ciphertext, ret);
}
...
- IMPLICIT_REJECTION_HASH_INPUT_SIZE= 1120
*/
static void decapsulate_unpacked_c8(
...
void libcrux_ml_kem_mlkem768_portable_decapsulate_unpacked(
    ...
    libcrux_ml_kem_mlkem768_MlKem768Ciphertext *ciphertext, uint8_t ret[32U]) {
  decapsulate_unpacked_c8(private_key, ciphertext, ret);
}
...
- ETA2_RANDOMNESS_SIZE= 128
*/
static tuple_3c encapsulate_e5(
...
tuple_3c libcrux_ml_kem_mlkem768_portable_encapsulate(
  ...
  uint8_t copy_of_randomness[32U];
  memcpy(copy_of_randomness, randomness, (size_t)32U * sizeof(uint8_t));
  return encapsulate_e5(uu____0, copy_of_randomness);
}
...
- ETA2_RANDOMNESS_SIZE= 128
*/
static tuple_3c encapsulate_unpacked_1f(
  ...
  uint8_t copy_of_randomness[32U];
  memcpy(copy_of_randomness, randomness, (size_t)32U * sizeof(uint8_t));
  return libcrux_ml_kem_ind_cca_unpacked_encapsulate_unpacked_88(
...
tuple_3c libcrux_ml_kem_mlkem768_portable_encapsulate_unpacked(
  ...
  uint8_t copy_of_randomness[32U];
  memcpy(copy_of_randomness, randomness, (size_t)32U * sizeof(uint8_t));
  return encapsulate_unpacked_1f(uu____0, copy_of_randomness);
}
...
- ETA1_RANDOMNESS_SIZE= 128
*/
static libcrux_ml_kem_mlkem768_MlKem768KeyPair generate_keypair_99(
...
generate_keypair_unpacked_2f(uint8_t randomness[64U]) {
  /* Passing arrays by value in Rust generates a copy in C */
  uint8_t copy_of_randomness[64U];
  memcpy(copy_of_randomness, randomness, (size_t)64U * sizeof(uint8_t));
  return libcrux_ml_kem_ind_cca_unpacked_generate_keypair_unpacked_0c(
      copy_of_randomness);
}
...
libcrux_ml_kem_mlkem768_portable_generate_key_pair_unpacked(
  ...
  uint8_t copy_of_randomness[64U];
  memcpy(copy_of_randomness, randomness, (size_t)64U * sizeof(uint8_t));
  return generate_keypair_unpacked_2f(copy_of_randomness);
}
...
- PUBLIC_KEY_SIZE= 1184
*/
static bool validate_public_key_0f(uint8_t *public_key) {
...
core_option_Option_92 libcrux_ml_kem_mlkem768_portable_validate_public_key(
  ...
  if (validate_public_key_0f(public_key.value)) {
