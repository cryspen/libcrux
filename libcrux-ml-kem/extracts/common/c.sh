...
if [[ "$no_charon" = 0 ]]; then
    ...
    (cd $repo_root/crates/utils/secrets && RUSTFLAGS="--cfg eurydice" $CHARON_HOME/bin/charon --remove-associated-types '*' --translate-all-methods)
    ...
    (cd $repo_root/crates/algorithms/sha3 && RUSTFLAGS="--cfg eurydice" $CHARON_HOME/bin/charon --remove-associated-types '*' --rustc-arg=-Cdebug-assertions=no)
