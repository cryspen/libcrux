...
TEST(Kyber1024TestPortable, ModifiedSecretKeyTest)
{
    ...
    auto sharedSecret3 = libcrux_ml_kem_mlkem1024_portable_decapsulate(&key_pair.sk, &ctxt.fst);
...
TEST(MlKem1024TestPortable, NISTKnownAnswerTest)
{
    ...
    for (auto kat : kats)
    {
        ...
        auto sk_hash = libcrux_sha3_sha256(
            mk_borrow_slice_u8(key_pair.sk.data, LIBCRUX_ML_KEM_MLKEM1024_SECRET_KEY_SIZE));
        EXPECT_EQ(0, memcmp(sk_hash.data, kat.sha3_256_hash_of_secret_key.data(), 32));

        auto ctxt = libcrux_ml_kem_mlkem1024_portable_encapsulate(&key_pair.pk, encaps_rand);
...
TEST(MlKem1024TestNeon, ConsistencyTest)
{
    ...
    auto ctxt = libcrux_ml_kem_mlkem1024_neon_encapsulate(&key_pair.pk, randomness);
