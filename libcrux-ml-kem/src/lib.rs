...
use libcrux_polynomials::{PortableVector, SIMD256Vector};
...
>(
    ...
    if libcrux_platform::simd256_support() {
        let deserialized_pk = deserialize_ring_elements_reduced::<PUBLIC_KEY_SIZE, K, SIMD256Vector>(
        ...
        *public_key == public_key_serialized
        ...
        let deserialized_pk = deserialize_ring_elements_reduced::<PUBLIC_KEY_SIZE, K, PortableVector>(
            &public_key[..RANKED_BYTES_PER_RING_ELEMENT],
        );
        let public_key_serialized = serialize_public_key::<
            K,
