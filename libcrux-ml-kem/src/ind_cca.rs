...
>(
    ...
    if valid {
        ...
        let seed = &public_key[ranked_bytes_per_ring_element(K)..];

        // ML_KEM_DIS_01: #value >= 20 in seed
        let counts = count_seed(seed);
        ...
        // ML_KEM_DIS_04: not more than 26, 31, 36 same elements in Zq in A'
        ...
        // ML_KEM_DIS_05: no more than 13 sequential elements in A
        valid &= long_sequence_in_a(&unpacked.ind_cpa_public_key.A);

        // ML_KEM_DIS_06: no more than 720, 1466, 2467 over or under 1664 in Z_q
...
fn count_seed(vector: &[u8]) -> std::collections::HashMap<u8, u64> {
    let mut counts = std::collections::HashMap::new();
