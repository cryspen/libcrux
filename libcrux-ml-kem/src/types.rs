...
pub mod unpacked {
    ...
    pub(crate) struct MlKemPrivateKeyUnpacked<const K: usize, Vector: Operations> {
        pub(crate) secret_as_ntt: [PolynomialRingElement<Vector>; K],
    }

    /// An unpacked ML-KEM Public Key
    pub(crate) struct MlKemPublicKeyUnpacked<const K: usize, Vector: Operations> {
    ...
    pub(crate) struct MlKemKeyPairUnpacked<const K: usize, Vector: Operations> {
