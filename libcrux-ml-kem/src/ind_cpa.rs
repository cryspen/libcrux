...
pub(crate) mod unpacked {
    ...
    impl<const K: usize, Vector: Operations> IndCpaPrivateKeyUnpacked<K, Vector> {
        #[hax_lib::requires(16 * 32 <= out.len())]
        pub fn as_bytes(&self, out: &mut [u8]) {
            #[cfg(hax)]
            let _out_len = out.len();

            for i in 0..self.secret_as_ntt.len() {
                hax_lib::loop_invariant!(|_i: usize| out.len() == _out_len);
                let offset = i * 512;
                self.secret_as_ntt[i].to_bytes(&mut out[offset..offset + 512]);
            }
        }
    }
