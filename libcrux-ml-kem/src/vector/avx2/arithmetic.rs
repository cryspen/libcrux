...
pub(crate) fn cond_subtract_3329(vector: Vec256) -> Vec256 {
    ...
    proof!(r#"assert (forall i. get_lane $field_modulus i == (mk_i16 3329))"#);
...
pub(crate) fn barrett_reduce(vector: Vec256) -> Vec256 {
    ...
    proof!(r#"assert (forall i. get_lane $t512 i == (mk_i16 512))"#);

    let t1 = mm256_add_epi16(t0, t512);

    proof!(r#"assert (forall i. get_lane $t1 i == get_lane $t0 i +. (mk_i16 512))"#);
...
pub(crate) fn montgomery_multiply_by_constant(vector: Vec256, constant: i16) -> Vec256 {
    ...
    proof!(r#"assert (forall i. get_lane $vec_constant i == $constant)"#);
    let value_low = mm256_mullo_epi16(vector, vec_constant);

    proof!(r#"assert (forall i. get_lane $value_low i == get_lane $vector i *. $constant)"#);
    let k = mm256_mullo_epi16(
        value_low,
        mm256_set1_epi16(INVERSE_OF_MODULUS_MOD_MONTGOMERY_R as i16),
    );

    proof!(r#"assert (forall i. get_lane $k i == get_lane $value_low i *. (neg (mk_i16 3327)))"#);

    let modulus = mm256_set1_epi16(FIELD_MODULUS);

    proof!(r#"assert (forall i. get_lane $modulus i == (mk_i16 3329))"#);
...
pub(crate) fn montgomery_multiply_by_constants(vec: Vec256, constants: Vec256) -> Vec256 {
    ...
    proof!("assert (forall i. get_lane $value_low i == get_lane $vec i *. get_lane $constants i)");
...
pub(crate) fn montgomery_multiply_m128i_by_constants(vec: Vec128, constants: Vec128) -> Vec128 {
    ...
    proof!(r#"assert (forall i. get_lane128 $modulus i == (mk_i16 3329))"#);
