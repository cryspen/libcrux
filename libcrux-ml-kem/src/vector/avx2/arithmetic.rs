...
pub(crate) fn add(lhs: Vec256, rhs: Vec256) -> Vec256 {
    ...
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane result i == get_lane lhs i +. get_lane rhs i);
...
pub(crate) fn multiply_by_constant(vector: Vec256, constant: i16) -> Vec256 {
    ...
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane result i == get_lane vector i *. constant);
...
#[inline(always)]
#[cfg_attr(hax, hax_lib::ensures(|result| fstar!("Libcrux_intrinsics.Avx2_extract.vec256_as_i16x16 $result == 
                           Spec.Utils.map_array (fun x -> x &. $constant) (Libcrux_intrinsics.Avx2_extract.vec256_as_i16x16 $vector)")))]
...
#[cfg_attr(hax, hax_lib::ensures(|result| fstar!("(v_SHIFT_BY >=. 0l /\\ v_SHIFT_BY <. 16l) ==> 
...
#[cfg_attr(hax, hax_lib::requires(fstar!("Spec.Utils.is_i16b_array (pow2 12 - 1) (Libcrux_intrinsics.Avx2_extract.vec256_as_i16x16 $vector)")))]
#[cfg_attr(hax, hax_lib::ensures(|result| fstar!("forall i. i < 16 ==> 
                get_lane $result i == 
...
pub(crate) fn cond_subtract_3329(vector: Vec256) -> Vec256 {
    ...
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $field_modulus i == 3329s)");
    ...
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $conditional_add_field_modulus i == (get_lane $sign_mask i &. 3329s))");

    let result = mm256_add_epi16(v_minus_field_modulus, conditional_add_field_modulus);
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $result i == (get_lane $v_minus_field_modulus i +. get_lane $conditional_add_field_modulus i));
...
pub(crate) fn barrett_reduce(vector: Vec256) -> Vec256 {
    ...
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $t0 i == (cast (((cast (get_lane $vector i) <: i32) *. (cast v_BARRETT_MULTIPLIER <: i32)) >>! 16l) <: i16))");
    let t512 = mm256_set1_epi16(512);
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $t512 i == 512s)");
    let t1 = mm256_add_epi16(t0, t512);
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $t1 i == get_lane $t0 i +. 512s)");
...
pub(crate) fn montgomery_multiply_by_constant(vector: Vec256, constant: i16) -> Vec256 {
    ...
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $vec_constant i == $constant)");
    let value_low = mm256_mullo_epi16(vector, vec_constant);
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $value_low i == get_lane $vector i *. $constant)");
    let k = mm256_mullo_epi16(
        value_low,
        mm256_set1_epi16(INVERSE_OF_MODULUS_MOD_MONTGOMERY_R as i16),
    );
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $k i == get_lane $value_low i *. (neg 3327s))");
    let modulus = mm256_set1_epi16(FIELD_MODULUS);
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane $modulus i == 3329s)");
    ...
    #[cfg(hax)]
    fstar!("Spec.Utils.lemma_range_at_percent 3329 (pow2 32);
...
pub(crate) fn montgomery_multiply_m128i_by_constants(vec: Vec128, constants: Vec128) -> Vec128 {
    ...
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane128 $value_low i == get_lane128 $vec i *. get_lane128 $constants i)");
    ...
    #[cfg(hax)]
    fstar!("assert (forall i. get_lane128 $modulus i == 3329s)");
