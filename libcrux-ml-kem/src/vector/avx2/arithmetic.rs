...
pub(crate) fn cond_subtract_3329(vector: Vec256) -> Vec256 {
    ...
    hax_lib::fstar!("assert (forall i. get_lane $field_modulus i == (mk_i16 3329))");
    ...
    hax_lib::fstar!("assert (forall i. get_lane $conditional_add_field_modulus i == (get_lane $sign_mask i &. (mk_i16 3329)))");
...
pub(crate) fn barrett_reduce(vector: Vec256) -> Vec256 {
    ...
    hax_lib::fstar!("assert (forall i. get_lane $t0 i == (cast (((cast (get_lane $vector i) <: i32) *. (cast v_BARRETT_MULTIPLIER <: i32)) >>! (mk_i32 16)) <: i16))");
    let t512 = mm256_set1_epi16(512);
    hax_lib::fstar!("assert (forall i. get_lane $t512 i == (mk_i16 512))");
    let t1 = mm256_add_epi16(t0, t512);
    hax_lib::fstar!("assert (forall i. get_lane $t1 i == get_lane $t0 i +. (mk_i16 512))");
...
pub(crate) fn montgomery_multiply_by_constant(vector: Vec256, constant: i16) -> Vec256 {
    ...
    hax_lib::fstar!("assert (forall i. get_lane $k i == get_lane $value_low i *. (neg (mk_i16 3327)))");
    let modulus = mm256_set1_epi16(FIELD_MODULUS);
    hax_lib::fstar!("assert (forall i. get_lane $modulus i == (mk_i16 3329))");
...
pub(crate) fn montgomery_multiply_m128i_by_constants(vec: Vec128, constants: Vec128) -> Vec128 {
    ...
    hax_lib::fstar!("assert (forall i. get_lane128 $modulus i == (mk_i16 3329))");
