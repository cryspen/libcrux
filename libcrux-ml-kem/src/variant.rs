...
impl Variant for MlKem {
    ...
    fn cpa_keygen_seed<Hasher: Hash>(key_generation_seed: &[u8], rank: usize, output: &mut [u8; 64]) {
        ...
        seed[CPA_PKE_KEY_GENERATION_SEED_SIZE] = rank as u8;
        hax_lib::fstar!(
            "Lib.Sequence.eq_intro #u8 #33 $seed
            (Seq.append $key_generation_seed (Seq.create 1 (cast $K <: u8)))"
        );
        Hasher::G(&seed, output)
    }
}
