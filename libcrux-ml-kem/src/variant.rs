...
pub(crate) trait Variant {
    ...
    #[cfg_attr(hax, hax_lib::ensures(|res| fstar!("$res == $shared_secret")))] // We only have post-conditions for ML-KEM, not Kyber
    fn kdf<const K: usize, const CIPHERTEXT_SIZE: usize, Hasher: Hash<K>>(
        shared_secret: &[u8],
        ciphertext: &MlKemCiphertext<CIPHERTEXT_SIZE>,
    ) -> [u8; 32];
    #[cfg_attr(hax, hax_lib::requires(randomness.len() == 32))]
    #[cfg_attr(hax, hax_lib::ensures(|res| fstar!("$res == $randomness")))] // We only have post-conditions for ML-KEM, not Kyber
...
impl Variant for MlKem {
    #[inline(always)]
    #[cfg_attr(hax, hax_lib::requires(shared_secret.len() == 32))]
    #[cfg_attr(hax, hax_lib::ensures(|res| fstar!("$res == $shared_secret")))]
    fn kdf<const K: usize, const CIPHERTEXT_SIZE: usize, Hasher: Hash<K>>(
        shared_secret: &[u8],
    ...
    #[inline(always)]
    #[cfg_attr(hax, hax_lib::requires(randomness.len() == 32))]
    #[cfg_attr(hax, hax_lib::ensures(|res| fstar!("$res == $randomness")))]
    fn entropy_preprocess<const K: usize, Hasher: Hash<K>>(randomness: &[u8]) -> [u8; 32] {
        let mut out = [0u8; 32];
