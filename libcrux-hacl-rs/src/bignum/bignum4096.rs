...
pub fn mul(a: &[u64], b: &[u64], res: &mut [u64]) {
    ...
    super::bignum::bn_karatsuba_mul_uint64(64u32, a, b, &mut tmp, res)
}
...
pub fn sqr(a: &[u64], res: &mut [u64]) {
    let mut tmp: [u64; 256] = [0u64; 256usize];
    super::bignum::bn_karatsuba_sqr_uint64(64u32, a, &mut tmp, res)
}
...
fn precompr2(nBits: u32, n: &[u64], res: &mut [u64]) {
    ...
    for _i in 0u32..8192u32.wrapping_sub(nBits) {
        ...
        super::bignum4096::add_mod(n, &a_copy, &b_copy, res)
    }
}
...
fn to(n: &[u64], nInv: u64, r2: &[u64], a: &[u64], aM: &mut [u64]) {
    ...
    super::bignum4096::reduction(n, nInv, &mut c, aM)
}
...
fn from(n: &[u64], nInv_u64: u64, aM: &[u64], a: &mut [u64]) {
    ...
    super::bignum4096::reduction(n, nInv_u64, &mut tmp, a)
}
...
fn areduction(n: &[u64], nInv: u64, c: &mut [u64], res: &mut [u64]) {
    ...
    let c00: u64 = (&c0)[0usize];
    let mut tmp: [u64; 64] = [0u64; 64usize];
    let c1: u64 = super::bignum4096::sub(res, n, &mut tmp);
...
fn amont_mul(n: &[u64], nInv_u64: u64, aM: &[u64], bM: &[u64], resM: &mut [u64]) {
    ...
    super::bignum4096::areduction(n, nInv_u64, &mut c, resM)
}
...
fn bn_slow_precomp(n: &[u64], mu: u64, r2: &[u64], a: &[u64], res: &mut [u64]) {
    ...
    super::bignum4096::areduction(n, mu, &mut a1, &mut a_mod);
    super::bignum4096::to(n, mu, r2, &a_mod, res)
}
...
pub fn r#mod(n: &[u64], a: &[u64], res: &mut [u64]) -> bool {
    ...
    if is_valid_m == 0xFFFFFFFFFFFFFFFFu64 {
        ...
        super::bignum4096::bn_slow_precomp(n, mu, &r2, a, res)
    } else {
        (res[0usize..64usize]).copy_from_slice(&[0u64; 64usize])
    };
...
fn exp_vartime_precomp(
    ...
    if bBits < 200u32 {
        ...
        super::bignum4096::to(n, mu, r2, a, &mut aM);
        ...
        super::bignum4096::from(ctx_r2.0, mu, ctx_r2.1, &mut resM);
        ...
        for i in 0u32..bBits {
            ...
            if bit != 0u64 {
                ...
                super::bignum4096::amont_mul(ctx_n0.1, mu, &aM_copy, &aM, &mut resM);
                lowstar::ignore::ignore::<&[u64]>(&ctx)
            };
            let mut aM_copy: [u64; 64] = [0u64; 64usize];
            ((&mut aM_copy)[0usize..64usize]).copy_from_slice(&(&aM)[0usize..64usize]);
            let ctx_n0: (&[u64], &[u64]) = ctx.split_at(0usize);
            super::bignum4096::amont_sqr(ctx_n0.1, mu, &aM_copy, &mut aM);
            lowstar::ignore::ignore::<&[u64]>(&ctx)
        }
        super::bignum4096::from(n, mu, &resM, res)
    } else {
        let mut aM: [u64; 64] = [0u64; 64usize];
        super::bignum4096::to(n, mu, r2, a, &mut aM);
        let mut resM: [u64; 64] = [0u64; 64usize];
        ...
        let ctx_n: (&[u64], &[u64]) = ctx.split_at(0usize);
        let ctx_r2: (&[u64], &[u64]) = ctx_n.1.split_at(64usize);
        super::bignum4096::from(ctx_r2.0, mu, ctx_r2.1, t1.0);
        lowstar::ignore::ignore::<&[u64]>(&ctx);
        ...
        krml::unroll_for!(7, "i", 0u32, 1u32, {
            ...
            let ctx_n0: (&[u64], &[u64]) = ctx.split_at(0usize);
            super::bignum4096::amont_sqr(ctx_n0.1, mu, &aM_copy, &mut tmp);
            lowstar::ignore::ignore::<&[u64]>(&ctx);
            ((&mut table)[2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(64u32) as usize
                ..2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(64u32) as usize + 64usize])
                .copy_from_slice(&(&tmp)[0usize..64usize]);
            let t2: (&[u64], &[u64]) = table
                .split_at(2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(64u32) as usize);
            ...
            super::bignum4096::amont_mul(ctx_n1.1, mu, &aM_copy0, t2.1, &mut tmp);
            lowstar::ignore::ignore::<&[u64]>(&ctx);
            ((&mut table)[2u32.wrapping_mul(i).wrapping_add(3u32).wrapping_mul(64u32) as usize
                ..2u32.wrapping_mul(i).wrapping_add(3u32).wrapping_mul(64u32) as usize + 64usize])
                .copy_from_slice(&(&tmp)[0usize..64usize])
        });
        if bBits.wrapping_rem(4u32) != 0u32 {
            ...
            super::bignum4096::from(ctx_r20.0, mu, ctx_r20.1, &mut resM);
            lowstar::ignore::ignore::<&[u64]>(&ctx)
        };
        ...
        for i in 0u32..bBits.wrapping_div(4u32) {
            krml::unroll_for!(4, "_i", 0u32, 1u32, {
                ...
                ((&mut aM_copy)[0usize..64usize]).copy_from_slice(&(&resM)[0usize..64usize]);
                let ctx_n0: (&[u64], &[u64]) = ctx.split_at(0usize);
                super::bignum4096::amont_sqr(ctx_n0.1, mu, &aM_copy, &mut resM);
                lowstar::ignore::ignore::<&[u64]>(&ctx)
            });
            let k: u32 = bBits
            ...
            ((&mut aM_copy)[0usize..64usize]).copy_from_slice(&(&resM)[0usize..64usize]);
            let ctx_n0: (&[u64], &[u64]) = ctx.split_at(0usize);
            super::bignum4096::amont_mul(ctx_n0.1, mu, &aM_copy, &tmp0, &mut resM);
            lowstar::ignore::ignore::<&[u64]>(&ctx)
        }
        super::bignum4096::from(n, mu, &resM, res)
    }
...
fn exp_consttime_precomp(
    ...
    if bBits < 200u32 {
        ...
        for i in 0u32..bBits {
            ...
            ((&mut aM_copy)[0usize..64usize]).copy_from_slice(&(&aM)[0usize..64usize]);
            let ctx_n0: (&[u64], &[u64]) = ctx.split_at(0usize);
            super::bignum4096::amont_mul(ctx_n0.1, mu, &aM_copy, &resM, &mut aM);
            lowstar::ignore::ignore::<&[u64]>(&ctx);
            let mut aM_copy0: [u64; 64] = [0u64; 64usize];
            ((&mut aM_copy0)[0usize..64usize]).copy_from_slice(&(&resM)[0usize..64usize]);
            let ctx_n1: (&[u64], &[u64]) = ctx.split_at(0usize);
            super::bignum4096::amont_sqr(ctx_n1.1, mu, &aM_copy0, &mut resM);
            lowstar::ignore::ignore::<&[u64]>(&ctx);
            (&mut sw)[0usize] = bit
        }
        ...
        if bBits.wrapping_rem(4u32) != 0u32 {
            let i: u32 = bBits.wrapping_div(4u32).wrapping_mul(4u32);
            let bits_c: u64 = super::bignum_base::bn_get_bits_u64(bLen, b, i, 4u32);
            ((&mut resM)[0usize..64usize])
                .copy_from_slice(&(&(&table)[0usize..] as &[u64])[0usize..64usize]);
...
fn exp_vartime(nBits: u32, n: &[u64], a: &[u64], bBits: u32, b: &[u64], res: &mut [u64]) {
    ...
    super::bignum4096::precompr2(nBits, n, &mut r2);
    let mu: u64 = super::bignum::mod_inv_uint64(n[0usize]);
    super::bignum4096::exp_vartime_precomp(n, mu, &r2, a, bBits, b, res)
}
...
fn exp_consttime(nBits: u32, n: &[u64], a: &[u64], bBits: u32, b: &[u64], res: &mut [u64]) {
    ...
    super::bignum4096::precompr2(nBits, n, &mut r2);
    let mu: u64 = super::bignum::mod_inv_uint64(n[0usize]);
    super::bignum4096::exp_consttime_precomp(n, mu, &r2, a, bBits, b, res)
}
...
pub fn mod_exp_vartime(n: &[u64], a: &[u64], bBits: u32, b: &[u64], res: &mut [u64]) -> bool {
    ...
    let nBits: u32 = 64u32.wrapping_mul(super::bignum_base::bn_get_top_index_u64(64u32, n) as u32);
    if is_valid_m == 0xFFFFFFFFFFFFFFFFu64 {
        super::bignum4096::exp_vartime(nBits, n, a, bBits, b, res)
    } else {
        (res[0usize..64usize]).copy_from_slice(&[0u64; 64usize])
    };
    is_valid_m == 0xFFFFFFFFFFFFFFFFu64
...
pub fn mod_exp_consttime(n: &[u64], a: &[u64], bBits: u32, b: &[u64], res: &mut [u64]) -> bool {
    ...
    let nBits: u32 = 64u32.wrapping_mul(super::bignum_base::bn_get_top_index_u64(64u32, n) as u32);
    if is_valid_m == 0xFFFFFFFFFFFFFFFFu64 {
        super::bignum4096::exp_consttime(nBits, n, a, bBits, b, res)
    } else {
        (res[0usize..64usize]).copy_from_slice(&[0u64; 64usize])
    };
    is_valid_m == 0xFFFFFFFFFFFFFFFFu64
...
pub fn mod_inv_prime_vartime(n: &[u64], a: &[u64], res: &mut [u64]) -> bool {
    ...
    if is_valid_m == 0xFFFFFFFFFFFFFFFFu64 {
        ...
        krml::unroll_for!(3, "i", 60u32, 1u32, {
        ...
        super::bignum4096::exp_vartime(nBits, n, a, 4096u32, &n2, res)
    } else {
        (res[0usize..64usize]).copy_from_slice(&[0u64; 64usize])
    };
    is_valid_m == 0xFFFFFFFFFFFFFFFFu64
...
pub fn mont_ctx_init(n: &[u64]) -> Box<[super::bignum::bn_mont_ctx_u64]> {
    ...
    let nBits: u32 = 64u32.wrapping_mul(super::bignum_base::bn_get_top_index_u64(64u32, n) as u32);
    super::bignum4096::precompr2(nBits, n, r21);
    let mu: u64 = super::bignum::mod_inv_uint64(n[0usize]);
...
pub fn mod_inv_prime_vartime_precomp(
    ...
    let c2: u64 = c1;
    lowstar::ignore::ignore::<u64>(c2);
    super::bignum4096::exp_vartime_precomp(n, mu, r2, a, 4096u32, &n2, res)
}
