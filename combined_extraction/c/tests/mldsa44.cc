...
TEST(MlDsa44TestPortable, ConsistencyTest) {
  // Generate key pair
  Eurydice_arr_ec keygen_rand = {0};
  memset(keygen_rand.data, 0x13, 32);

  auto key_pair = libcrux_ml_dsa_ml_dsa_44_portable_generate_key_pair(keygen_rand);
  ...
  auto signature_result = libcrux_ml_dsa_ml_dsa_44_portable_sign(
  ...
  // Verify
  auto result = libcrux_ml_dsa_ml_dsa_44_portable_verify(
...
TEST(MlDsa44TestAvx2, ConsistencyTest) {
  Eurydice_arr_ec keygen_rand = {0};
  memset(keygen_rand.data, 0x13, 32);
  auto key_pair = libcrux_ml_dsa_ml_dsa_44_avx2_generate_key_pair(keygen_rand);
  ...
  auto msg_slice = mk_borrow_slice_u8((uint8_t *)msg, 79);
  auto context_slice = mk_borrow_slice_u8((uint8_t *)context, 3);
  auto signature_result = libcrux_ml_dsa_ml_dsa_44_avx2_sign(
      &key_pair.signing_key, msg_slice, context_slice, sign_rand);
  EXPECT_EQ(signature_result.tag, core_result_Ok);
  auto signature = signature_result.val.case_Ok;

  // Verify
  auto result = libcrux_ml_dsa_ml_dsa_44_avx2_verify(
      &key_pair.verification_key, msg_slice, context_slice, &signature);
...
TEST(MlDsa44TestPortable, NISTKnownAnswerTest) {
  // XXX: This should be done in a portable way.
  auto kats = read_kats("tests/nistkats-44.json");
  ...
  for (auto kat : kats) {
    ...
    auto vk_hash =
        libcrux_sha3_sha256(mk_borrow_slice_u8(key_pair.verification_key.data, 1312U));
    EXPECT_EQ(0, memcmp(vk_hash.data,
                        kat.sha3_256_hash_of_verification_key.data(), 32));

    auto sk_hash =
        libcrux_sha3_sha256(mk_borrow_slice_u8(key_pair.signing_key.data, 2560U));
    EXPECT_EQ(
    ...
    auto sig_hash =
        libcrux_sha3_sha256(mk_borrow_slice_u8(signature.data, 2420U));
    EXPECT_EQ(0,
...
#ifdef LIBCRUX_X64
TEST(MlDsa44TestAvx2, NISTKnownAnswerTest) {
  // XXX: This should be done in a portable way.
  auto kats = read_kats("tests/nistkats-44.json");
