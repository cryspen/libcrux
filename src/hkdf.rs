...
pub fn extract(alg: Algorithm, salt: &[u8], ikm: &[u8]) -> Vec<u8> {
    match alg {
        Algorithm::Sha256 => hacl::hazmat::hkdf::sha2_256::extract(salt, ikm).into(),
        Algorithm::Sha384 => hacl::hazmat::hkdf::sha2_384::extract(salt, ikm).into(),
        Algorithm::Sha512 => hacl::hazmat::hkdf::sha2_512::extract(salt, ikm).into(),
    }
}
...
pub fn expand(alg: Algorithm, prk: &[u8], info: &[u8], okm_len: usize) -> Result<Vec<u8>, Error> {
    match alg {
        Algorithm::Sha256 => hacl::hazmat::hkdf::sha2_256::vec::expand(prk, info, okm_len)
            .map_err(|_| Error::OkmLengthTooLarge),
        Algorithm::Sha384 => hacl::hazmat::hkdf::sha2_384::vec::expand(prk, info, okm_len)
            .map_err(|_| Error::OkmLengthTooLarge),
        Algorithm::Sha512 => hacl::hazmat::hkdf::sha2_512::vec::expand(prk, info, okm_len)
            .map_err(|_| Error::OkmLengthTooLarge),
    }
}
