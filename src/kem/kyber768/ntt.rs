...
pub(crate) mod kyber_polynomial_ring_element_mod {
    use crate::kem::kyber768::{
        arithmetic::{montgomery_reduce, barrett_reduce, fe_mul, KyberFieldElement, KyberPolynomialRingElement},
        ...
        for layer in NTT_LAYERS.iter().rev() {
            for offset in (0..(COEFFICIENTS_IN_RING_ELEMENT - layer)).step_by(2 * layer) {
                ...
                for j in offset..offset + layer {
                    let t = montgomery_reduce(i32::from(re[j + layer]) * ZETAS_MONTGOMERY_DOMAIN[zeta_i]);
                    re[j + layer] = re[j] - t;
                    re[j] += t;
                }
            }
