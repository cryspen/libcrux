...
use self::{
    ...
    kyber::{decapsulate, encapsulate, generate_keypair},
    parameters::{
        CPA_PKE_PUBLIC_KEY_SIZE_768, CPA_PKE_SECRET_KEY_SIZE_768,
        RANKED_BYTES_PER_RING_ELEMENT_768, RANK_768,
    },
};
...
mod kyber {
    ...
    ) -> Result<KeyPair<PRIVATE_KEY_SIZE, PUBLIC_KEY_SIZE>, BadRejectionSamplingRandomnessError>
    {
        ...
        let ((ind_cpa_private_key, public_key), sampling_a_error) =
            ind_cpa::generate_keypair::<
                K,
    ...
    >(
        ...
        let (ind_cpa_public_key_hash, implicit_rejection_value) =
            secret_key.split_at(H_DIGEST_SIZE);
