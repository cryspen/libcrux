...
pub fn sqr(a: &[u32], res: &mut [u32]) {
    ...
    let r: u32 = super::bignum_base::bn_add_eq_len_u32(16u32, &a_copy, &b_copy, res);
    let c0: u32 = r;
    ...
    let r0: u32 = super::bignum_base::bn_add_eq_len_u32(16u32, &a_copy0, &b_copy0, res);
    let c1: u32 = r0;
...
fn precompr2(nBits: u32, n: &[u32], res: &mut [u32]) {
    ...
    for _i in 0u32..512u32.wrapping_sub(nBits) {
        ...
        super::bignum256_32::add_mod(n, &a_copy, &b_copy, res)
    }
}
...
fn to(n: &[u32], nInv: u32, r2: &[u32], a: &[u32], aM: &mut [u32]) {
    ...
    super::bignum256_32::mul(a, r2, &mut c);
    super::bignum256_32::reduction(n, nInv, &mut c, aM)
}
...
fn from(n: &[u32], nInv_u64: u32, aM: &[u32], a: &mut [u32]) {
    ...
    super::bignum256_32::reduction(n, nInv_u64, &mut tmp, a)
}
...
fn areduction(n: &[u32], nInv: u32, c: &mut [u32], res: &mut [u32]) {
    ...
    let c00: u32 = (&c0)[0usize];
    let mut tmp: [u32; 8] = [0u32; 8usize];
    let c1: u32 = super::bignum256_32::sub(res, n, &mut tmp);
    lowstar::ignore::ignore::<u32>(c1);
...
fn amont_mul(n: &[u32], nInv_u64: u32, aM: &[u32], bM: &[u32], resM: &mut [u32]) {
    let mut c: [u32; 16] = [0u32; 16usize];
    super::bignum256_32::mul(aM, bM, &mut c);
    super::bignum256_32::areduction(n, nInv_u64, &mut c, resM)
}

#[inline]
fn amont_sqr(n: &[u32], nInv_u64: u32, aM: &[u32], resM: &mut [u32]) {
    let mut c: [u32; 16] = [0u32; 16usize];
    super::bignum256_32::sqr(aM, &mut c);
    super::bignum256_32::areduction(n, nInv_u64, &mut c, resM)
}
...
fn bn_slow_precomp(n: &[u32], mu: u32, r2: &[u32], a: &[u32], res: &mut [u32]) {
    ...
    super::bignum256_32::areduction(n, mu, &mut a1, &mut a_mod);
    super::bignum256_32::to(n, mu, r2, &a_mod, res)
}
...
fn exp_vartime_precomp(
    ...
    if bBits < 200u32 {
        ...
        super::bignum256_32::to(n, mu, r2, a, &mut aM);
        ...
        super::bignum256_32::from(ctx_r2.0, mu, ctx_r2.1, &mut resM);
        ...
        for i in 0u32..bBits {
            ...
            if bit != 0u32 {
                ...
                super::bignum256_32::amont_mul(ctx_n0.1, mu, &aM_copy, &aM, &mut resM);
                lowstar::ignore::ignore::<&[u32]>(&ctx)
            };
            let mut aM_copy: [u32; 8] = [0u32; 8usize];
            ((&mut aM_copy)[0usize..8usize]).copy_from_slice(&(&aM)[0usize..8usize]);
            let ctx_n0: (&[u32], &[u32]) = ctx.split_at(0usize);
            super::bignum256_32::amont_sqr(ctx_n0.1, mu, &aM_copy, &mut aM);
            lowstar::ignore::ignore::<&[u32]>(&ctx)
        }
        super::bignum256_32::from(n, mu, &resM, res)
    } else {
        let mut aM: [u32; 8] = [0u32; 8usize];
        super::bignum256_32::to(n, mu, r2, a, &mut aM);
        let mut resM: [u32; 8] = [0u32; 8usize];
        ...
        let ctx_n: (&[u32], &[u32]) = ctx.split_at(0usize);
        let ctx_r2: (&[u32], &[u32]) = ctx_n.1.split_at(8usize);
        super::bignum256_32::from(ctx_r2.0, mu, ctx_r2.1, t1.0);
        lowstar::ignore::ignore::<&[u32]>(&ctx);
        ...
        krml::unroll_for!(7, "i", 0u32, 1u32, {
            ...
            let ctx_n0: (&[u32], &[u32]) = ctx.split_at(0usize);
            super::bignum256_32::amont_sqr(ctx_n0.1, mu, &aM_copy, &mut tmp);
            lowstar::ignore::ignore::<&[u32]>(&ctx);
            ((&mut table)[2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(8u32) as usize
                ..2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(8u32) as usize + 8usize])
                .copy_from_slice(&(&tmp)[0usize..8usize]);
            let t2: (&[u32], &[u32]) =
                table.split_at(2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(8u32) as usize);
            ...
            super::bignum256_32::amont_mul(ctx_n1.1, mu, &aM_copy0, t2.1, &mut tmp);
            lowstar::ignore::ignore::<&[u32]>(&ctx);
            ((&mut table)[2u32.wrapping_mul(i).wrapping_add(3u32).wrapping_mul(8u32) as usize
                ..2u32.wrapping_mul(i).wrapping_add(3u32).wrapping_mul(8u32) as usize + 8usize])
                .copy_from_slice(&(&tmp)[0usize..8usize])
        });
        if bBits.wrapping_rem(4u32) != 0u32 {
            ...
            let a_bits_l: (&[u32], &[u32]) = table.split_at(bits_l32.wrapping_mul(8u32) as usize);
            ((&mut resM)[0usize..8usize]).copy_from_slice(&a_bits_l.1[0usize..8usize])
        } else {
            let ctx_n0: (&[u32], &[u32]) = ctx.split_at(0usize);
            let ctx_r20: (&[u32], &[u32]) = ctx_n0.1.split_at(8usize);
            super::bignum256_32::from(ctx_r20.0, mu, ctx_r20.1, &mut resM);
            lowstar::ignore::ignore::<&[u32]>(&ctx)
        };
        ...
        for i in 0u32..bBits.wrapping_div(4u32) {
            krml::unroll_for!(4, "_i", 0u32, 1u32, {
                ...
                ((&mut aM_copy)[0usize..8usize]).copy_from_slice(&(&resM)[0usize..8usize]);
                let ctx_n0: (&[u32], &[u32]) = ctx.split_at(0usize);
                super::bignum256_32::amont_sqr(ctx_n0.1, mu, &aM_copy, &mut resM);
                lowstar::ignore::ignore::<&[u32]>(&ctx)
            });
            let k: u32 = bBits
            ...
            ((&mut aM_copy)[0usize..8usize]).copy_from_slice(&(&resM)[0usize..8usize]);
            let ctx_n0: (&[u32], &[u32]) = ctx.split_at(0usize);
            super::bignum256_32::amont_mul(ctx_n0.1, mu, &aM_copy, &tmp0, &mut resM);
            lowstar::ignore::ignore::<&[u32]>(&ctx)
        }
        super::bignum256_32::from(n, mu, &resM, res)
    }
...
fn exp_consttime_precomp(
    ...
    if bBits < 200u32 {
        ...
        for i in 0u32..bBits {
            ...
            krml::unroll_for!(8, "i0", 0u32, 1u32, {
                let dummy: u32 =
            ...
            ((&mut aM_copy)[0usize..8usize]).copy_from_slice(&(&aM)[0usize..8usize]);
            let ctx_n0: (&[u32], &[u32]) = ctx.split_at(0usize);
            super::bignum256_32::amont_mul(ctx_n0.1, mu, &aM_copy, &resM, &mut aM);
            lowstar::ignore::ignore::<&[u32]>(&ctx);
            let mut aM_copy0: [u32; 8] = [0u32; 8usize];
            ((&mut aM_copy0)[0usize..8usize]).copy_from_slice(&(&resM)[0usize..8usize]);
            let ctx_n1: (&[u32], &[u32]) = ctx.split_at(0usize);
            super::bignum256_32::amont_sqr(ctx_n1.1, mu, &aM_copy0, &mut resM);
            lowstar::ignore::ignore::<&[u32]>(&ctx);
            (&mut sw)[0usize] = bit
        }
        ...
        if bBits.wrapping_rem(4u32) != 0u32 {
            let i: u32 = bBits.wrapping_div(4u32).wrapping_mul(4u32);
            let bits_c: u32 = super::bignum_base::bn_get_bits_u32(bLen, b, i, 4u32);
            ((&mut resM)[0usize..8usize])
                .copy_from_slice(&(&(&table)[0usize..] as &[u32])[0usize..8usize]);
            krml::unroll_for!(15, "i0", 0u32, 1u32, {
                ...
                krml::unroll_for!(8, "i1", 0u32, 1u32, {
...
pub fn mod_exp_vartime(n: &[u32], a: &[u32], bBits: u32, b: &[u32], res: &mut [u32]) -> bool {
    ...
    let nBits: u32 = 32u32.wrapping_mul(super::bignum_base::bn_get_top_index_u32(8u32, n));
    if is_valid_m == 0xFFFFFFFFu32 {
        super::bignum256_32::exp_vartime(nBits, n, a, bBits, b, res)
    } else {
        (res[0usize..8usize]).copy_from_slice(&[0u32; 8usize])
    };
    is_valid_m == 0xFFFFFFFFu32
...
pub fn mod_exp_consttime(n: &[u32], a: &[u32], bBits: u32, b: &[u32], res: &mut [u32]) -> bool {
    ...
    let nBits: u32 = 32u32.wrapping_mul(super::bignum_base::bn_get_top_index_u32(8u32, n));
    if is_valid_m == 0xFFFFFFFFu32 {
        super::bignum256_32::exp_consttime(nBits, n, a, bBits, b, res)
    } else {
        (res[0usize..8usize]).copy_from_slice(&[0u32; 8usize])
    };
    is_valid_m == 0xFFFFFFFFu32
...
pub fn mod_inv_prime_vartime(n: &[u32], a: &[u32], res: &mut [u32]) -> bool {
    ...
    if is_valid_m == 0xFFFFFFFFu32 {
        ...
        super::bignum256_32::exp_vartime(nBits, n, a, 256u32, &n2, res)
    } else {
        (res[0usize..8usize]).copy_from_slice(&[0u32; 8usize])
    };
    is_valid_m == 0xFFFFFFFFu32
...
pub fn mod_inv_prime_vartime_precomp(
    ...
    let c2: u32 = c1;
    lowstar::ignore::ignore::<u32>(c2);
    super::bignum256_32::exp_vartime_precomp(n, mu, r2, a, 256u32, &n2, res)
}
