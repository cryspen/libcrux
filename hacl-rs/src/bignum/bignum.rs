...
pub(crate) fn bn_karatsuba_mul_uint32(
    ...
    if aLen < 32u32 || aLen.wrapping_rem(2u32) == 1u32 {
        super::bignum_base::bn_mul_u32(aLen, a, aLen, b, res)
    } else {
        ...
        let c0: u32 = super::bignum_base::bn_sub_eq_len_u32(len2, a1.0, a1.1, tmp·.1);
        let c1: u32 = super::bignum_base::bn_sub_eq_len_u32(len2, a1.1, a1.0, t1.0);
        for i in 0u32..len2 {
            let x: u32 = 0u32.wrapping_sub(c0) & t1.0[i as usize]
        ...
        let c00: u32 = c0;
        let c01: u32 = super::bignum_base::bn_sub_eq_len_u32(len2, b1.0, b1.1, tmp·.1);
        let c10: u32 = super::bignum_base::bn_sub_eq_len_u32(len2, b1.1, b1.0, tmp·.0);
        for i in 0u32..len2 {
            let x: u32 = 0u32.wrapping_sub(c01) & tmp·.0[i as usize]
        ...
        let t45: (&mut [u32], &mut [u32]) = t231
            .1
            .split_at_mut(2u32.wrapping_mul(aLen) as usize - aLen as usize);
        let t67: (&mut [u32], &mut [u32]) = t45
            .1
            .split_at_mut(3u32.wrapping_mul(aLen) as usize - 2u32.wrapping_mul(aLen) as usize);
        let c2: u32 = super::bignum_base::bn_add_eq_len_u32(aLen, r231.0, r231.1, t231.0);
        let c_sign: u32 = c00 ^ c11;
        let c3: u32 = super::bignum_base::bn_sub_eq_len_u32(aLen, t231.0, t45.0, t67.1);
        let c31: u32 = c2.wrapping_sub(c3);
        let c4: u32 = super::bignum_base::bn_add_eq_len_u32(aLen, t231.0, t45.0, t67.0);
        ...
        let r1: u32 = super::bignum_base::bn_add_eq_len_u32(aLen, &a_copy, &b_copy, r.1);
        let r10: u32 = r1;
        let c: u32 = r10;
        let c6: u32 = c;
        ...
        let c010: u32 =
            lib::inttypes_intrinsics::add_carry_u32(0u32, r0.1[0usize], c7, &mut r0.1[0usize..]);
        let r11: u32 = if 1u32
            < aLen
                .wrapping_add(aLen)
                .wrapping_sub(aLen.wrapping_add(aLen2))
        {
                ...
                .wrapping_div(4u32)
            {
                ...
                (&mut c8)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u32((&c8)[0usize], t11, 0u32, res_i.1);
                ...
                (&mut c8)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u32((&c8)[0usize], t110, 0u32, res_i0.1);
                ...
                (&mut c8)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u32((&c8)[0usize], t111, 0u32, res_i1.1);
                ...
                (&mut c8)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u32((&c8)[0usize], t112, 0u32, res_i2.1)
            }
            for i in aLen
                .wrapping_add(aLen)
                .wrapping_sub(aLen.wrapping_add(aLen2))
...
pub(crate) fn bn_karatsuba_mul_uint64(
    ...
    if aLen < 32u32 || aLen.wrapping_rem(2u32) == 1u32 {
        super::bignum_base::bn_mul_u64(aLen, a, aLen, b, res)
    } else {
        let len2: u32 = aLen.wrapping_div(2u32);
        ...
        let c0: u64 = super::bignum_base::bn_sub_eq_len_u64(len2, a1.0, a1.1, tmp·.1);
        let c1: u64 = super::bignum_base::bn_sub_eq_len_u64(len2, a1.1, a1.0, t1.0);
        for i in 0u32..len2 {
            let x: u64 = 0u64.wrapping_sub(c0) & t1.0[i as usize]
        ...
        let c00: u64 = c0;
        let c01: u64 = super::bignum_base::bn_sub_eq_len_u64(len2, b1.0, b1.1, tmp·.1);
        let c10: u64 = super::bignum_base::bn_sub_eq_len_u64(len2, b1.1, b1.0, tmp·.0);
        for i in 0u32..len2 {
            let x: u64 = 0u64.wrapping_sub(c01) & tmp·.0[i as usize]
        ...
        let t45: (&mut [u64], &mut [u64]) = t231
            .1
            .split_at_mut(2u32.wrapping_mul(aLen) as usize - aLen as usize);
        let t67: (&mut [u64], &mut [u64]) = t45
            .1
            .split_at_mut(3u32.wrapping_mul(aLen) as usize - 2u32.wrapping_mul(aLen) as usize);
        let c2: u64 = super::bignum_base::bn_add_eq_len_u64(aLen, r231.0, r231.1, t231.0);
        let c_sign: u64 = c00 ^ c11;
        let c3: u64 = super::bignum_base::bn_sub_eq_len_u64(aLen, t231.0, t45.0, t67.1);
        let c31: u64 = c2.wrapping_sub(c3);
        let c4: u64 = super::bignum_base::bn_add_eq_len_u64(aLen, t231.0, t45.0, t67.0);
        ...
        ((&mut a_copy)[0usize..aLen as usize]).copy_from_slice(&r.1[0usize..aLen as usize]);
        ((&mut b_copy)[0usize..aLen as usize]).copy_from_slice(&t67.0[0usize..aLen as usize]);
        let r1: u64 = super::bignum_base::bn_add_eq_len_u64(aLen, &a_copy, &b_copy, r.1);
        let r10: u64 = r1;
        let c: u64 = r10;
        let c6: u64 = c;
        ...
        let c010: u64 =
            lib::inttypes_intrinsics::add_carry_u64(0u64, r0.1[0usize], c7, &mut r0.1[0usize..]);
        let r11: u64 = if 1u32
            < aLen
                .wrapping_add(aLen)
                .wrapping_sub(aLen.wrapping_add(aLen2))
...
pub(crate) fn bn_karatsuba_sqr_uint32(aLen: u32, a: &[u32], tmp: &mut [u32], res: &mut [u32]) {
    if aLen < 32u32 || aLen.wrapping_rem(2u32) == 1u32 {
        ...
        let c0: u32 = super::bignum_base::bn_sub_eq_len_u32(len2, a1.0, a1.1, tmp·.1);
        let c1: u32 = super::bignum_base::bn_sub_eq_len_u32(len2, a1.1, a1.0, tmp·.0);
        for i in 0u32..len2 {
            let x: u32 = 0u32.wrapping_sub(c0) & tmp·.0[i as usize]
                | !0u32.wrapping_sub(c0) & tmp·.1[i as usize];
            let os: (&mut [u32], &mut [u32]) = tmp·.0.split_at_mut(0usize);
            os.1[i as usize] = x
        }
        ...
        let t45: (&mut [u32], &mut [u32]) = t231
            ...
            .split_at_mut(2u32.wrapping_mul(aLen) as usize - aLen as usize);
        let c2: u32 = super::bignum_base::bn_add_eq_len_u32(aLen, r231.0, r231.1, t231.0);
        let c3: u32 = super::bignum_base::bn_sub_eq_len_u32(aLen, t231.0, t45.0, t45.1);
                ...
                .wrapping_sub(aLen.wrapping_add(aLen2))
        {
                ...
                .wrapping_div(4u32)
            {
                ...
                let res_i: (&mut [u32], &mut [u32]) =
                    res1.1.split_at_mut(4u32.wrapping_mul(i) as usize);
                (&mut c4)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u32((&c4)[0usize], t1, 0u32, res_i.1);
                let t10: u32 = res1.1[4u32.wrapping_mul(i).wrapping_add(1u32) as usize];
                let res_i0: (&mut [u32], &mut [u32]) =
                    res1.1.split_at_mut(4u32.wrapping_mul(i) as usize + 1usize);
                (&mut c4)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u32((&c4)[0usize], t10, 0u32, res_i0.1);
                let t11: u32 = res1.1[4u32.wrapping_mul(i).wrapping_add(2u32) as usize];
                let res_i1: (&mut [u32], &mut [u32]) =
                    res1.1.split_at_mut(4u32.wrapping_mul(i) as usize + 2usize);
                (&mut c4)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u32((&c4)[0usize], t11, 0u32, res_i1.1);
                let t12: u32 = res1.1[4u32.wrapping_mul(i).wrapping_add(3u32) as usize];
                let res_i2: (&mut [u32], &mut [u32]) =
                    res1.1.split_at_mut(4u32.wrapping_mul(i) as usize + 3usize);
                (&mut c4)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u32((&c4)[0usize], t12, 0u32, res_i2.1)
            }
            for i in aLen
                .wrapping_add(aLen)
                .wrapping_sub(aLen.wrapping_add(aLen2))
...
pub(crate) fn bn_karatsuba_sqr_uint64(aLen: u32, a: &[u64], tmp: &mut [u64], res: &mut [u64]) {
    if aLen < 32u32 || aLen.wrapping_rem(2u32) == 1u32 {
        ...
        let c0: u64 = super::bignum_base::bn_sub_eq_len_u64(len2, a1.0, a1.1, tmp·.1);
        let c1: u64 = super::bignum_base::bn_sub_eq_len_u64(len2, a1.1, a1.0, tmp·.0);
        for i in 0u32..len2 {
            let x: u64 = 0u64.wrapping_sub(c0) & tmp·.0[i as usize]
                | !0u64.wrapping_sub(c0) & tmp·.1[i as usize];
            let os: (&mut [u64], &mut [u64]) = tmp·.0.split_at_mut(0usize);
            os.1[i as usize] = x
        }
        ...
        let t45: (&mut [u64], &mut [u64]) = t231
            ...
            .split_at_mut(2u32.wrapping_mul(aLen) as usize - aLen as usize);
        let c2: u64 = super::bignum_base::bn_add_eq_len_u64(aLen, r231.0, r231.1, t231.0);
        let c3: u64 = super::bignum_base::bn_sub_eq_len_u64(aLen, t231.0, t45.0, t45.1);
                ...
                .wrapping_sub(aLen.wrapping_add(aLen2))
        {
                ...
                .wrapping_div(4u32)
            {
                ...
                let res_i: (&mut [u64], &mut [u64]) =
                    res1.1.split_at_mut(4u32.wrapping_mul(i) as usize);
                (&mut c4)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u64((&c4)[0usize], t1, 0u64, res_i.1);
                let t10: u64 = res1.1[4u32.wrapping_mul(i).wrapping_add(1u32) as usize];
                let res_i0: (&mut [u64], &mut [u64]) =
                    res1.1.split_at_mut(4u32.wrapping_mul(i) as usize + 1usize);
                (&mut c4)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u64((&c4)[0usize], t10, 0u64, res_i0.1);
                let t11: u64 = res1.1[4u32.wrapping_mul(i).wrapping_add(2u32) as usize];
                let res_i1: (&mut [u64], &mut [u64]) =
                    res1.1.split_at_mut(4u32.wrapping_mul(i) as usize + 2usize);
                (&mut c4)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u64((&c4)[0usize], t11, 0u64, res_i1.1);
                let t12: u64 = res1.1[4u32.wrapping_mul(i).wrapping_add(3u32) as usize];
                let res_i2: (&mut [u64], &mut [u64]) =
                    res1.1.split_at_mut(4u32.wrapping_mul(i) as usize + 3usize);
                (&mut c4)[0usize] =
                    lib::inttypes_intrinsics::add_carry_u64((&c4)[0usize], t12, 0u64, res_i2.1)
            }
            for i in aLen
                .wrapping_add(aLen)
                .wrapping_sub(aLen.wrapping_add(aLen2))
...
pub fn bn_add_mod_n_u32(len1: u32, n: &[u32], a: &[u32], b: &[u32], res: &mut [u32]) {
    ...
    for i in len1.wrapping_div(4u32).wrapping_mul(4u32)..len1 {
        ...
        (&mut c1)[0usize] = lib::inttypes_intrinsics::sub_borrow_u32((&c1)[0usize], t1, t2, res_i.1)
    }
...
pub fn mod_inv_uint32(n0: u32) -> u32 {
    ...
    krml::unroll_for!(32, "_i", 0u32, 1u32, {
        ...
        (&mut ub)[0usize] = (us ^ beta_if_u_is_odd)
            .wrapping_shr(1u32)
...
pub fn bn_check_modulus_u32(len: u32, n: &[u32]) -> u32 {
    ...
    ((&mut one)[0usize..len as usize])
        .copy_from_slice(&vec![0u32; len as usize].into_boxed_slice());
    (&mut one)[0usize] = 1u32;
...
pub fn bn_check_modulus_u64(len: u32, n: &[u64]) -> u64 {
    ...
    ((&mut one)[0usize..len as usize])
        .copy_from_slice(&vec![0u64; len as usize].into_boxed_slice());
    (&mut one)[0usize] = 1u64;
...
pub(crate) fn bn_almost_mont_reduction_u32(
    ...
    let c00: u32 = (&c0)[0usize];
    let mut tmp: Box<[u32]> = vec![0u32; len as usize].into_boxed_slice();
    let c1: u32 = super::bignum_base::bn_sub_eq_len_u32(len, res, n, &mut tmp);
    lowstar::ignore::ignore::<u32>(c1);
...
pub(crate) fn bn_almost_mont_reduction_u64(
    ...
    let c00: u64 = (&c0)[0usize];
    let mut tmp: Box<[u64]> = vec![0u64; len as usize].into_boxed_slice();
    let c1: u64 = super::bignum_base::bn_sub_eq_len_u64(len, res, n, &mut tmp);
    lowstar::ignore::ignore::<u64>(c1);
...
pub(crate) fn bn_mod_exp_vartime_precomp_u32(
    ...
    if bBits < 200u32 {
        ...
        ((&mut ctx)[len as usize..len as usize + len as usize])
        ...
        krml::unroll_for!(7, "i", 0u32, 1u32, {
            ...
            ((&mut table)[2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(len) as usize
                ..2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(len) as usize
                    + len as usize])
                .copy_from_slice(&(&tmp)[0usize..len as usize]);
            let t2: (&[u32], &[u32]) =
