...
| `montgomery_multiply` | ✅ (rewritten in `int` 04fd066f0) | ✅ (impl closed; Step 9) | ✅ (AVX2 impl scaffolded; Step 11 Track 4) | per-lane `(v lhs * v rhs * 8265825) % q` | Portable impl discharges via `lemma_montgomery_multiply_lane_intro` after revealing `Spec.MLDSA.Math.mod_q`; AVX2 impl now uses Track-4 `lemma_mont_mul_bound_and_mod_q` (admit body — ML-KEM-style i32 Montgomery proof deferred to USER lane) — structurally complete, no top-level body admit |
