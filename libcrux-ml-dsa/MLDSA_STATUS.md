...
**Tip**: Step 10 close (2026-04-28). Track A: impl-side `#[ensures]/[requires]` lifted on both Portable and AVX2 impls to discharge the cherry-picked array-level conjuncts (decompose, compute_hint, use_hint, power2round, reduce, gamma1_deserialize, t0_deserialize, t1_serialize); new lemma `lemma_power2round_t1_bound` in `Hacspec_ml_dsa.Commute.Chunk`; F-2 trait-side type-fix for the decompose `v gamma2` cast.  Track B: 5 non-trivial impl bodies (`infinity_norm_exceeds`, `montgomery_multiply` Portable, `shift_left_then_reduce`, `power2round`, `reduce`) extracted to top-level `*_with_proof` wrappers; impl methods are one-liners.  AVX2 `impl_1` perf dropped from 46.42s/637q (Track A) to 4.47s/1q (Track B); Portable `impl_1` dropped out of top-10.
...
**Empirical baseline (Track A+B)**: **98 modules invoked, [CHECK]=40, [ADMIT]=58, 98 verified, 0 errors, 0 make-level failures**.  The Track-A re-extraction surfaced that the prior `71/13/58/0` baseline was using stale `.fstar-cache/checked` files predating cherry-pick `a331580ec`; once the cache invalidates, F\* needed the impl posts lifted to discharge the strengthened trait posts.
