...
#[hax_lib::requires(fstar!(r#"v $bound > 0 /\ 
        (forall i. forall j. Spec.Utils.is_i32b_array_opaque 
            ...
            (i1._super_1586179615636010033.f_repr (Seq.index (Seq.index vector i).f_simd_units j)))"#))]
...
#[hax_lib::requires(fstar!(r#"v $SHIFT_BY == 13 /\ 
        (forall i. forall j.
            v (Seq.index (i1._super_1586179615636010033.f_repr (Seq.index re.f_simd_units i)) j) >= 0 /\
            v (Seq.index (i1._super_1586179615636010033.f_repr (Seq.index re.f_simd_units i)) j) <= 261631)"#))]
...
#[hax_lib::requires(fstar!(r#"${t.len()} == ${t1.len()} /\
    (forall i. forall j. 
        ...
        (v ${crate::simd::traits::specs::FIELD_MAX}) 
        (i1._super_1586179615636010033.f_repr (Seq.index (Seq.index t i).f_simd_units j)))"#))]
...
#[hax_lib::requires(fstar!(r#"
        (v $gamma2 == v ${crate::constants::GAMMA2_V261_888} \/ 
         ...
         (forall i. forall j. 
            Spec.Utils.is_i32b_array_opaque 
            (v ${crate::simd::traits::specs::FIELD_MAX}) 
            (i1._super_1586179615636010033.f_repr (Seq.index (Seq.index re_vector i).f_simd_units j)))"#))]
