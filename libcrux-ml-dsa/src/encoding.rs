...
pub(crate) mod t0 {
    use crate::{
        ...
        constants::{
            BITS_IN_LOWER_PART_OF_T, BYTES_FOR_RING_ELEMENT_OF_T0S, BYTES_FOR_RING_ELEMENT_OF_T1S,
        },
    };
    ...
    #[inline(always)]
    pub(crate) fn serialize(re: PolynomialRingElement) -> [u8; BYTES_FOR_RING_ELEMENT_OF_T0S] {
...
pub(crate) mod t1 {
    use crate::{arithmetic::PolynomialRingElement, constants::BYTES_FOR_RING_ELEMENT_OF_T1S};
    #[inline(always)]
    pub(crate) fn serialize(re: PolynomialRingElement) -> [u8; BYTES_FOR_RING_ELEMENT_OF_T1S] {
...
pub(crate) mod error {
    ...
    #[inline(always)]
    fn serialize_when_eta_is_2<const BYTES_FOR_OUTPUT: usize>(
    ...
    #[inline(always)]
    fn serialize_when_eta_is_4<const BYTES_FOR_OUTPUT: usize>(
        re: PolynomialRingElement,
    ) -> [u8; BYTES_FOR_OUTPUT] {
        let mut serialized = [0u8; BYTES_FOR_OUTPUT];
        let ETA: i32 = 4;
    ...
    #[inline(always)]
    pub(crate) fn serialize<const ETA: usize, const BYTES_FOR_OUTPUT: usize>(
        re: PolynomialRingElement,
    ) -> [u8; BYTES_FOR_OUTPUT] {
        match ETA {
            2 => serialize_when_eta_is_2::<BYTES_FOR_OUTPUT>(re),
            4 => serialize_when_eta_is_4::<BYTES_FOR_OUTPUT>(re),
            _ => unreachable!(),
        }
    }
...
// ---------------------------------------------------------------------------
// Functions for serializing and deserializing a ring element of w1s.
// ---------------------------------------------------------------------------

pub(crate) mod w1 {
    use crate::arithmetic::PolynomialRingElement;

    pub(crate) fn serialize<const GAMMA2: usize, const BYTES_FOR_OUTPUT: usize>(
        re: PolynomialRingElement,
    ) -> [u8; BYTES_FOR_OUTPUT] {
...
mod tests {
    ...
    fn test_deserialize_to_mask_when_gamma1_is_2_pow_17() {
        ...
        assert_eq!(
            mask::deserialize::<17>(&bytes).coefficients,
            expected_coefficients
        );
    }
    ...
    fn test_deserialize_to_mask_when_gamma1_is_2_pow_19() {
        ...
        assert_eq!(
            mask::deserialize::<19>(&bytes).coefficients,
            expected_coefficients
        );
    }

    #[test]
    fn test_serialize_w1() {
        ...
        assert_eq!(w1::serialize::<95_232, 192>(re), serialized);
        ...
        assert_eq!(w1::serialize::<261_888, 128>(re), serialized);
    }
}
