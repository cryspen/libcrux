...
pub(super) fn compute_hint(low: &Vec256, high: &Vec256, gamma2: i32, hint: &mut Vec256) -> usize {
    ...
    *hint = mm256_and_si256(*hint, mm256_set1_epi32(0x1));
        ...
        let aux (i: u64{v i < 8}) : Lemma
          (ensures
            ...
            ((v (to_i32x8 ${high} i) >= 0 /\ v (to_i32x8 ${high} i) < 8380417) ==>
              v (to_i32x8 ${hint} i) ==
              Spec.MLDSA.Math.compute_one_hint (v (to_i32x8 ${low} i)) (v (to_i32x8 ${high} i)) (v $gamma2))) =
          Libcrux_ml_dsa.Simd.Avx2.Arithmetic_theory.lemma_and_one_binary ((to_i32x8 ${low_within_bound} i) |.
              (to_i32x8 ${low_equals_minus_gamma2_and_high_is_nonzero} i));
          Libcrux_ml_dsa.Simd.Avx2.Arithmetic_theory.lemma_ones_zero_v ();
          if (v (to_i32x8 ${high} i) >= 0 && v (to_i32x8 ${high} i) < 8380417)
          then
            Libcrux_ml_dsa.Simd.Avx2.Arithmetic_theory.lemma_or_and_mask_bit (to_i32x8 ${low_within_bound} i)
              (to_i32x8 ${low_equals_minus_gamma2_and_high_is_nonzero} i)
          else () in
    ...
    proof!(
        ...
        =
          Libcrux_ml_dsa.Simd.Avx2.Arithmetic_theory.lemma_or_sign_and (to_i32x8 ${low_within_bound} i)
            (to_i32x8 ${low_equals_minus_gamma2_and_high_is_nonzero} i)
        in
        introduce
...
pub(super) fn use_hint(gamma2: Gamma2, r: &Vec256, hint: &mut Vec256) {
    ...
    let aux (i: u64{v i < 8})
      ...
      Libcrux_ml_dsa.Simd.Avx2.Arithmetic_theory.lemma_use_hint_value ${gamma2} (v (to_i32x8 ${r0} i)) (v (to_i32x8 ${r1} i))
