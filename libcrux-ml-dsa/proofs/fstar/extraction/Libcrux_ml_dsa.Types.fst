...
let impl_1 (v_SIZE: usize) : Core.Clone.t_Clone (t_MLDSASigningKey v_SIZE) =
  { f_clone = (fun x -> x) }
...
let impl_3 (v_SIZE: usize) : Core.Clone.t_Clone (t_MLDSAVerificationKey v_SIZE) =
  { f_clone = (fun x -> x) }
...
let impl_5 (v_SIZE: usize) : Core.Clone.t_Clone (t_MLDSASignature v_SIZE) =
  { f_clone = (fun x -> x) }
