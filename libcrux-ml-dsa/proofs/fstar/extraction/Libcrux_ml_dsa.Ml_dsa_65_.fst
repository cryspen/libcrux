...
let generate_key_pair (randomness: t_Array u8 (mk_usize 32)) =
  ...
  in
  let verification_key:t_Array u8 (mk_usize 1952) =
    Rust_primitives.Hax.repeat (mk_u8 0) (mk_usize 1952)
  in
  let tmp0, tmp1:(t_Array u8 (mk_usize 4032) & t_Array u8 (mk_usize 1952)) =
    Libcrux_ml_dsa.Ml_dsa_generic.Multiplexing.Ml_dsa_65_.generate_key_pair randomness
      ...
      verification_key
  in
  let signing_key:t_Array u8 (mk_usize 4032) = tmp0 in
  let verification_key:t_Array u8 (mk_usize 1952) = tmp1 in
  let _:Prims.unit = () in
  {
    Libcrux_ml_dsa.Types.f_signing_key = Libcrux_ml_dsa.Types.impl__new (mk_usize 4032) signing_key;
    Libcrux_ml_dsa.Types.f_verification_key
    =
    Libcrux_ml_dsa.Types.impl_2__new (mk_usize 1952) verification_key
  }
  <:
...
let sign
     ...
     =
  Libcrux_ml_dsa.Ml_dsa_generic.Multiplexing.Ml_dsa_65_.sign (Libcrux_ml_dsa.Types.impl__as_ref (mk_usize
          4032)
        signing_key
      <:
      t_Array u8 (mk_usize 4032))
    message
    context
    randomness
