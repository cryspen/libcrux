module Libcrux_ml_dsa.Simd.Avx2.Invntt_theory
...
(* ============================================================================
   Hand-written companion: the F* theory formerly inlined in the
   `#[hax_lib::fstar::before(...)]` blocks of `src/simd/avx2/invntt.rs` -- the
   within-chunk inverse layer 0/1/2 congruence theory, the `outer_3_plus`
   invariant algebra, and the cross-chunk layer 3..7 transport + scaling theory.

   Decls are copied BYTE-EXACT from the green extracted
   Libcrux_ml_dsa.Simd.Avx2.Invntt.fst.  Every relocated region was push/pop
   balanced on its own, so no outer `#push-options` had to be reconstructed.

   WHAT DID *NOT* MOVE, and why: `lemma_establish_chunk_scaled_avx2` and
   `lemma_inv_scale_step` mention `scale_montgomery_avx2__v_FACTOR` -- hax's
   hoisting of the `const FACTOR` declared *inside* the impl fn
   `scale_montgomery_avx2`.  That constant is defined in
   Libcrux_ml_dsa.Simd.Avx2.Invntt itself, so theory naming it cannot be
   compiled ahead of that module.  Both lemmas are called only from that fn's
   own inline `fstar!` blocks, and nothing else in this theory depends on them,
   so they stay inline in invntt.rs (each already sat in its own balanced
   push/pop region).  Contrast the Portable mirror, whose impl passes the
   literal `41_978`, so Portable.Invntt_theory states the same facts over
   `mk_i32 41978` and needs no impl constant.

   SCOPE NOTE: the aliases `T` (Avx2NttTheory), `C` (Hacspec_ml_dsa.Commute.Chunk),
   `FN` (Libcrux_ml_dsa.Simd.Avx2.Ntt_theory) and `PI`
   (Libcrux_ml_dsa.Simd.Portable.Invntt_theory), plus `open Spec.MLDSA.Math`,
   are declared inside the relocated regions below and travel with them.  F*
   module abbreviations are file-scoped and are NOT re-exported by `open`, so
   Libcrux_ml_dsa.Simd.Avx2.Invntt re-declares each in the thin before-block it
   keeps.

   NOTE: `Avx2NttTheory` and `Libcrux_ml_dsa.Simd.Avx2.Ntt_theory` export
   several decls sharing a NAME but with DISTINCT identity (chunks_of_re_avx2,
   is_i32b_poly_avx2, forall32_elim_1d, ...).  This module consumes them ONLY
   through the `T.` / `FN.` aliases and never `open`s either, exactly as the
   inline theory did.  Do not "unify" them -- they are different opaque atoms.
