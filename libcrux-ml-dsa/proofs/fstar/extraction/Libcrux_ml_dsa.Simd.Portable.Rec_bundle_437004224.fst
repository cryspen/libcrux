...
let simd_unit_ntt_at_layer_0_
     ...
     =
  let t:i32 =
    montgomery_multiply_fe_by_fer (simd_unit.Libcrux_ml_dsa.Simd.Portable.Vector_type.f_coefficients.[
          sz 1 ]
        <:
        i32)
      zeta0
...
let serialize977980603 (simd_unit: Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit) =
...
let serialize300254843 (simd_unit: Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit) =
...
let rec add (lhs rhs: Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit) =
...
and montgomery_multiply (lhs rhs: Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit) =
...
and power2round (simd_unit: Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit) =
...
and subtract (lhs rhs: Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit) =
...
and deserialize_when_eta_is_2_ (serialized: t_Slice u8) =
  let _:Prims.unit =
    if true
    then
      let _:Prims.unit =
...
and deserialize_when_eta_is_4_ (serialized: t_Slice u8) =
  let _:Prims.unit =
    if true
    then
      let _:Prims.unit =
...
and deserialize154437703 (v_ETA: usize) (serialized: t_Slice u8) =
...
and deserialize_when_gamma1_is_2_pow_17_ (serialized: t_Slice u8) =
  let _:Prims.unit =
    if true
    then
      let _:Prims.unit =
...
and deserialize_when_gamma1_is_2_pow_19_ (serialized: t_Slice u8) =
  let _:Prims.unit =
    if true
    then
      let _:Prims.unit =
...
and deserialize244287932 (v_GAMMA1_EXPONENT: usize) (serialized: t_Slice u8) =
...
and deserialize297775919 (serialized: t_Slice u8) =
  let _:Prims.unit =
    if true
    then
      let _:Prims.unit =
  ...
  let simd_unit:Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit =
    {
      ...
      Rust_primitives.Hax.Monomorphized_update_at.update_at_usize simd_unit
        ...
        (sz 0)
        ((simd_unit.Libcrux_ml_dsa.Simd.Portable.Vector_type.f_coefficients.[ sz 0 ] <: i32) &.
          v_BITS_IN_LOWER_PART_OF_T_MASK
          <:
          i32)
    }
    <:
...
and deserialize960784460 (serialized: t_Slice u8) =
  let _:Prims.unit =
    if true
    then
      let _:Prims.unit =
...
and ntt (re: t_Array Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit (sz 32)) =
            ...
            Rust_primitives.Hax.Folds.fold_range offset
              ...
              (fun re j ->
                  ...
                  =
                    Rust_primitives.Hax.Monomorphized_update_at.update_at_usize re
                      j
                      (add (re.[ j ] <: Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit)
                          t
                        <:
                        Libcrux_ml_dsa.Simd.Portable.Vector_type.t_PortableSIMDUnit)
                  in
