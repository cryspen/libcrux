...
let ntt_at_layer_3_plus (v_LAYER zeta_i: usize) (re: t_Array u8 (sz 32)) =
  let step:usize = sz 1 <<! v_LAYER in
  let re, zeta_i:(t_Array u8 (sz 32) & usize) =
    Rust_primitives.Hax.Folds.fold_range (sz 0)
      (sz 128 >>! v_LAYER <: usize)
      (fun temp_0_ temp_1_ ->
          let re, zeta_i:(t_Array u8 (sz 32) & usize) = temp_0_ in
          let _:usize = temp_1_ in
          true)
      (re, zeta_i <: (t_Array u8 (sz 32) & usize))
      (fun temp_0_ round ->
          let re, zeta_i:(t_Array u8 (sz 32) & usize) = temp_0_ in
  ...
  zeta_i, re <: (usize & t_Array u8 (sz 32))

let ntt_at_layer_0_ (zeta_i: usize) (re: t_Array u8 (sz 32)) =
  let zeta_i:usize = zeta_i +! sz 1 in
  let re, zeta_i:(t_Array u8 (sz 32) & usize) =
    Rust_primitives.Hax.Folds.fold_range_step_by (sz 0)
      ...
      (fun temp_0_ round ->
          ...
          let round:usize = round in
          let a, b:(u8 & u8) =
            butterfly_2_ (re.[ round ] <: u8) (re.[ round +! sz 1 <: usize ] <: u8)
              (Libcrux_ml_dsa.Simd.Traits.v_ZETAS_TIMES_MONTGOMERY_R.[ zeta_i ] <: i32)
...
let ntt_at_layer_1_ (zeta_i: usize) (re: t_Array u8 (sz 32)) =
  let zeta_i:usize = zeta_i +! sz 1 in
  let re, zeta_i:(t_Array u8 (sz 32) & usize) =
    Rust_primitives.Hax.Folds.fold_range_step_by (sz 0)
      (Core.Slice.impl__len #u8 (re <: t_Slice u8) <: usize)
      ...
      (fun temp_0_ round ->
          ...
          let round:usize = round in
          let a, b:(u8 & u8) =
            butterfly_4_ (re.[ round ] <: u8)
              (re.[ round +! sz 1 <: usize ] <: u8)
              (Libcrux_ml_dsa.Simd.Traits.v_ZETAS_TIMES_MONTGOMERY_R.[ zeta_i ] <: i32)
              (Libcrux_ml_dsa.Simd.Traits.v_ZETAS_TIMES_MONTGOMERY_R.[ zeta_i +! sz 1 <: usize ]
                <:
                i32)
...
let ntt_at_layer_2_ (zeta_i: usize) (re: t_Array u8 (sz 32)) =
  let re, zeta_i:(t_Array u8 (sz 32) & usize) =
    Rust_primitives.Hax.Folds.fold_range_step_by (sz 0)
      (Core.Slice.impl__len #u8 (re <: t_Slice u8) <: usize)
      (sz 2)
...
let ntt (re: t_Array u8 (sz 32)) =
  ...
  let tmp0, tmp1:(usize & t_Array u8 (sz 32)) = ntt_at_layer_3_plus (sz 7) zeta_i re in
  let zeta_i:usize = tmp0 in
  let re:t_Array u8 (sz 32) = tmp1 in
  let _:Prims.unit = () in
  let tmp0, tmp1:(usize & t_Array u8 (sz 32)) = ntt_at_layer_3_plus (sz 6) zeta_i re in
  let zeta_i:usize = tmp0 in
  let re:t_Array u8 (sz 32) = tmp1 in
  let _:Prims.unit = () in
  let tmp0, tmp1:(usize & t_Array u8 (sz 32)) = ntt_at_layer_3_plus (sz 5) zeta_i re in
  let zeta_i:usize = tmp0 in
  let re:t_Array u8 (sz 32) = tmp1 in
  let _:Prims.unit = () in
  let tmp0, tmp1:(usize & t_Array u8 (sz 32)) = ntt_at_layer_3_plus (sz 4) zeta_i re in
  let zeta_i:usize = tmp0 in
  let re:t_Array u8 (sz 32) = tmp1 in
  let _:Prims.unit = () in
  let tmp0, tmp1:(usize & t_Array u8 (sz 32)) = ntt_at_layer_3_plus (sz 3) zeta_i re in
  let zeta_i:usize = tmp0 in
  let re:t_Array u8 (sz 32) = tmp1 in
  let _:Prims.unit = () in
  let tmp0, tmp1:(usize & t_Array u8 (sz 32)) = ntt_at_layer_2_ zeta_i re in
  let zeta_i:usize = tmp0 in
  let re:t_Array u8 (sz 32) = tmp1 in
  let _:Prims.unit = () in
  let tmp0, tmp1:(usize & t_Array u8 (sz 32)) = ntt_at_layer_1_ zeta_i re in
  let zeta_i:usize = tmp0 in
  let re:t_Array u8 (sz 32) = tmp1 in
  let _:Prims.unit = () in
  let tmp0, tmp1:(usize & t_Array u8 (sz 32)) = ntt_at_layer_0_ zeta_i re in
  let zeta_i:usize = tmp0 in
  let re:t_Array u8 (sz 32) = tmp1 in
  let _:Prims.unit = () in
  re
