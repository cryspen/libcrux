...
let montgomery_reduce_element (value: i64)
  ...
  in
  let _:Prims.unit = reveal_opaque (`%Spec.MLDSA.Math.mod_q) (Spec.MLDSA.Math.mod_q) in
  res
      ...
      (ensures
        fun simd_unit_future ->
          let simd_unit_future:Libcrux_ml_dsa.Simd.Portable.Vector_type.t_Coefficients =
            simd_unit_future
          in
          Spec.Utils.is_i32b_array_opaque 8380416 simd_unit_future.f_values /\
          Spec.MLDSA.Math.(forall i.
              i < 8 ==>
              mod_q (v (Seq.index simd_unit_future.f_values i)) ==
          ...
          (forall j.
              ...
              (let vecj = Seq.index simd_unit.f_values j in
                (Spec.Utils.is_i32b 8380416 vecj /\
                  Spec.MLDSA.Math.mod_q (v vecj) ==
                  Spec.MLDSA.Math.mod_q (v (Seq.index e_simd_unit0.f_values j) * v c * 8265825)))) /\
          (forall j.
...
let montgomery_multiply (lhs rhs: Libcrux_ml_dsa.Simd.Portable.Vector_type.t_Coefficients)
    : Prims.Pure Libcrux_ml_dsa.Simd.Portable.Vector_type.t_Coefficients
      ...
      (ensures
        fun lhs_future ->
          let lhs_future:Libcrux_ml_dsa.Simd.Portable.Vector_type.t_Coefficients = lhs_future in
          Spec.Utils.is_i32b_array_opaque 8380416 lhs_future.f_values /\
          Spec.MLDSA.Math.(forall i.
              i < 8 ==>
              mod_q (v (Seq.index lhs_future.f_values i)) ==
      ...
      (fun lhs i ->
          ...
          (forall j.
              ...
              (let vecj = Seq.index lhs.f_values j in
                (Spec.Utils.is_i32b 8380416 vecj /\
                  Spec.MLDSA.Math.mod_q (v vecj) ==
                  Spec.MLDSA.Math.mod_q (v (Seq.index e_lhs0.f_values j) *
                      v (Seq.index rhs.f_values j) *
                      8265825)))) /\
    ...
    : Prims.Pure bool
      ...
      (ensures
        fun result ->
          ...
          result == false ==> Spec.Utils.is_i32b_array (v bound - 1) simd_unit.f_values) =
      ...
      (fun simd_unit i ->
          ...
          (forall j.
              ...
              (Spec.Utils.is_i32b 8380416 (Seq.index simd_unit.f_values j) /\
                ...
                Spec.MLDSA.Math.mod_q (v ((Seq.index e_simd_unit0.f_values j) <<! v_SHIFT_BY)))) /\
          (forall j.
              j >= v i ==>
...
let compute_hint
  ...
  let _:Prims.unit =
    Spec.Utils.eq_repeati0 (sz 0) (Spec.MLDSA.Math.hint_counter hint.f_values) (0)
  in
  let hint, one_hints_count:(Libcrux_ml_dsa.Simd.Portable.Vector_type.t_Coefficients & usize) =
    Rust_primitives.Hax.Folds.fold_range (mk_usize 0)
      ...
      (fun temp_0_ i ->
          ...
          v one_hints_count ==
          Spec.Utils.repeati (i) (Spec.MLDSA.Math.hint_counter hint.f_values) (0))
