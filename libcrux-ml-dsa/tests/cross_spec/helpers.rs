...
//! coefficient/polynomial generators, and lane <-> scalar adapters between
//! the libcrux SIMD impls and a flat `[i32; 256]`.
...
// ---------------------------------------------------------------------------
// SIMD <-> scalar adapters.
// ---------------------------------------------------------------------------

/// Build a single SIMD unit from 8 coefficients.
...
/// Extract the 8 coefficients held in a SIMD unit.
...
/// Spread 256 flat coefficients across the 32 SIMD units of a ring element.
pub fn to_polynomial_units<O: Operations>(coeffs: &[i32; N]) -> [O; SIMD_UNITS] {
    let mut units = [O::zero(); SIMD_UNITS];
    for (u, unit) in units.iter_mut().enumerate() {
        let mut lane = [0i32; LANES];
        lane.copy_from_slice(&coeffs[u * LANES..(u + 1) * LANES]);
        *unit = to_simd_unit::<O>(&lane);
    }
    units
}

/// Flatten the 32 SIMD units of a ring element back to 256 coefficients.
pub fn from_polynomial_units<O: Operations>(units: &[O; SIMD_UNITS]) -> [i32; N] {
    let mut out = [0i32; N];
    for (u, unit) in units.iter().enumerate() {
        let lane = from_simd_unit::<O>(unit);
        out[u * LANES..(u + 1) * LANES].copy_from_slice(&lane);
    }
    out
}
