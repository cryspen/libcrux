        ...
        fn $name() {
            ...
            for kat in nist_kats {
                ...
                let verification_key_hash =
                    libcrux_sha3::sha256(key_pair.verification_key.as_raw());
                assert_eq!(
                    verification_key_hash, kat.sha3_256_hash_of_verification_key,
                    "verification_key_hash != kat.sha3_256_hash_of_verification_key"
                );

                let signing_key_hash = libcrux_sha3::sha256(key_pair.signing_key.as_raw());
                assert_eq!(
                ...
                let signature_hash = libcrux_sha3::sha256(signature.as_raw());
                assert_eq!(
