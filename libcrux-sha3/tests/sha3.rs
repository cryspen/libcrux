...
mod shake256 {
    ...
    mod neon {
        ...
        fn incremental() {
            ...
            let mut shake = incremental::init();
            incremental::shake256_absorb_final(&mut shake, b"Hello, World!", b"Hello, World!");

            let mut shaken0 = [0u8; 136]; // 1 block
            let mut shaken1 = [0u8; 136]; // 1 block
            incremental::shake256_squeeze_first_block(&mut shake, &mut shaken0, &mut shaken1);
            ...
            let mut shaken0 = [0u8; 136]; // 1 block
            let mut shaken1 = [0u8; 136]; // 1 block
            incremental::shake256_squeeze_next_block(&mut shake, &mut shaken0, &mut shaken1);
    ...
    mod xof {
        use libcrux_sha3::portable::incremental::{self, Xof};
