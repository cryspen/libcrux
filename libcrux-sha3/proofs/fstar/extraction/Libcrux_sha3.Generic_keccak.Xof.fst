...
let impl_1__fill_buffer
    ...
    : Prims.Pure (t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE & usize)
      (requires
        ...
        (Core.Num.impl_usize__MAX -!
          ...
          <:
          usize) &&
        self.f_buf_len <=. (Core.Num.impl_usize__MAX -! v_RATE <: usize))
      (fun _ -> Prims.l_True) =
  ...
  let self, hax_temp_output:(t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE & usize) =
    ...
    then
      ...
      in
      let self:t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE =
        { self with f_buf_len = self.f_buf_len +! consumed }
        <:
        t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE
      in
...
let impl_1__absorb_full
    ...
    : Prims.Pure (t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE & usize)
      ...
      (ensures
        fun temp_0_ ->
          let self_e_future, ret:(t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE & usize) =
            temp_0_
          in
          ret <. v_RATE && 
  ...
  let self:t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE =
    ...
    then
      ...
      in
      let _:Prims.unit = assume (i1.f_load_block_pre v_RATE self.f_inner borrowed (mk_usize 0)) in
      let self:t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE =
        {
          self with
          f_inner
  ...
  in
  assert (self.f_buf_len <=. v_RATE);
  let inputs_len:usize = Core.Slice.impl__len #u8 (inputs.[ mk_usize 0 ] <: t_Slice u8) in
  ...
  let self:t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE =
    Rust_primitives.Hax.Folds.fold_range (mk_usize 0)
      ...
      (fun self temp_1_ ->
          let self:t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE = self in
          let _:usize = temp_1_ in
          (self.f_buf_len =. buf_len <: bool) &&
          ((Core.Slice.impl__len #u8 (inputs.[ mk_usize 0 ] <: t_Slice u8) <: usize) =. inputs_len
            <:
            bool))
      self
      (fun self i ->
          ...
          let i:usize = i in
          assert (self.f_buf_len <=. v_RATE);
          let _:Prims.unit = assume (v input_consumed + v i * v v_RATE <= v inputs_len) in
          let _:Prims.unit =
            assume (i1.f_load_block_pre v_RATE self.f_inner inputs (input_consumed +! (i *! v_RATE))
              )
          in
          let self:t_KeccakXofState v_PARALLEL_LANES v_RATE v_STATE =
            {
