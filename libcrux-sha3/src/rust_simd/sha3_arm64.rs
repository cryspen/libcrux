...
#[inline(always)]
fn and_not(a: uint64x2_t, b: uint64x2_t) -> uint64x2_t {
    unsafe { vbicq_u64(a, b) }
}

#[inline(always)]
fn load_u64(v: u64) -> uint64x2_t {
    unsafe { vdupq_n_u64(v) }
}

#[inline(always)]
fn store(out: &mut [u8], val: uint64x2_t) {
    unsafe { vst1q_u64(out.as_mut_ptr() as *mut u64, val) }
}

#[allow(dead_code)]
#[inline(always)]
fn store64(out: &mut [u64], val: uint64x2_t) {
    unsafe { vst1q_u64(out.as_mut_ptr(), val) }
}

#[inline(always)]
fn load_bytes_16(bytes: &[u8]) -> uint64x2_t {
    unsafe { vld1q_u64(bytes.as_ptr() as *const u64) }
}

#[inline(always)]
fn load_u64_2(bytes: &[u64]) -> uint64x2_t {
    unsafe { vld1q_u64(bytes.as_ptr()) }
}

#[inline(always)]
fn transpose_primary(a: uint64x2_t, b: uint64x2_t) -> uint64x2_t {
    // let mut ap = [0u64; 2];
    // store64(&mut ap, a);
    // let mut bp = [0u64; 2];
    // store64(&mut bp, b);
...
#[inline(always)]
fn transpose_secondary(a: uint64x2_t, b: uint64x2_t) -> uint64x2_t {
    // let mut ap = [0u64; 2];
    // store64(&mut ap, a);
    // let mut bp = [0u64; 2];
    // store64(&mut bp, b);
