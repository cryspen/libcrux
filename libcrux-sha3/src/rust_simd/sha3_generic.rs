...
pub(crate) fn theta_rho<const N:usize, T:KeccakItem<N>>(s: &mut KeccakState<N,T>) {
    let c: [T; 5] = core::array::from_fn(|j| T::xor5(s.st[0][j],s.st[1][j],s.st[2][j],s.st[3][j],s.st[4][j]));
    let t : [T; 5] = core::array::from_fn(|j| T::rotate_left1_and_xor(c[(j+4)%5], c[(j+1)%5]));
...
#[inline(always)]
pub(crate) fn absorb_block<const N:usize, T:KeccakItem<N>,const RATE:usize>(s: &mut KeccakState<N,T>, blocks: [&[u8];N]) {
...
pub(crate) fn absorb_final<const N:usize, T:KeccakItem<N>,const RATE:usize, const DELIM:u8>(
        s: &mut KeccakState<N,T>, last: [&[u8];N]) {
...
#[inline(always)]
pub(crate) fn squeeze_first_block<const N:usize, T:KeccakItem<N>,const RATE:usize>(s: &KeccakState<N,T>, out: [&mut [u8];N]) {
    T::store_block::<RATE>(&s.st, out)
}

#[inline(always)]
pub(crate) fn squeeze_next_block<const N:usize, T:KeccakItem<N>,const RATE:usize>(s: &mut KeccakState<N,T>, out: [&mut [u8];N]) {
    keccakf1600(s);
    T::store_block::<RATE>(&s.st, out)
}
...
pub(crate) fn squeeze_first_three_blocks<const N:usize, T:KeccakItem<N>,const RATE:usize>(
        s: &mut KeccakState<N,T>, out: [&mut [u8];N]) {
...
#[inline(always)]
pub(crate) fn squeeze_last<const N:usize, T:KeccakItem<N>,const RATE:usize>(mut s: KeccakState<N,T>, out: [&mut [u8];N]) {
    keccakf1600(&mut s);
...
#[inline(always)]
pub(crate) fn squeeze_first_and_last<const N:usize, T:KeccakItem<N>,const RATE:usize>(s: &KeccakState<N,T>, out: [&mut [u8];N]) {
    let b = T::store_block_full::<RATE>(&s.st);
    for i in 0..N {
        out[i].copy_from_slice(&b[i][0..out[i].len()]);
    }
...
#[inline(always)]
pub(crate) fn keccak<const N:usize, T:KeccakItem<N>,const RATE:usize, const DELIM:u8>(data: [&[u8]; N], out: [&mut [u8]; N]) {
    ...
    if blocks == 0 {
        ...
        if last < outlen {squeeze_last::<N,T,RATE>(s, o1)}
    }
}
