    ...
    for super::P256
{
    fn generate_secret(
        ...
        <Self as libcrux_traits::ecdh::arrayref::ECDHArrayref<RAND_LEN, SECRET_LEN, PUBLIC_LEN>>::validate_secret(secret).map_err(|_| libcrux_traits::ecdh::arrayref::ECDHError::GenerateSecret)
    }
    ...
    fn secret_to_public(
        ...
        crate::p256::dh_initiator(public, secret.declassify_ref().as_slice())
            .then_some(())
            .ok_or(libcrux_traits::ecdh::arrayref::ECDHError::SecretToPublic)
    }
