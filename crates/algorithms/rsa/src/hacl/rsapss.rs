...
#[inline]
fn hash(a: crate::hacl::streaming_types::hash_alg, mHash: &mut [u8], msgLen: u32, msg: &[u8]) {
    match a {
...
fn mgf_hash(
    ...
    let hLen: u32 = crate::hacl::rsapss::hash_len(a);
    let n: u32 = maskLen
        .wrapping_sub(1u32)
        .wrapping_div(hLen)
        .wrapping_add(1u32);
    ...
    for i in 0u32..n {
        ...
        c.1[3usize] = i as u8;
        crate::hacl::rsapss::hash(a, acc_i.1, len.wrapping_add(4u32), &mgfseed_counter)
    }
    (res[0usize..maskLen as usize]).copy_from_slice(&(&(&acc)[0usize..])[0usize..maskLen as usize])
}

#[inline]
fn check_num_bits_u64(bs: u32, b: &[u64]) -> u64 {
...
#[inline]
fn check_modulus_u64(modBits: u32, n: &[u64]) -> u64 {
    ...
    let res: u64 = (&acc)[0usize];
    let m1: u64 = res;
    let m2: u64 = crate::hacl::rsapss::check_num_bits_u64(modBits, n);
    m0 & (m1 & m2)
}

#[inline]
fn check_exponent_u64(eBits: u32, e: &[u64]) -> u64 {
    let eLen: u32 = eBits
        .wrapping_sub(1u32)
        .wrapping_div(64u32)
        .wrapping_add(1u32);
    ...
    let res: u64 = mask1;
    let m0: u64 = res;
    let m1: u64 = crate::hacl::rsapss::check_num_bits_u64(eBits, e);
    !m0 & m1
}
...
fn pss_encode(
    ...
    crate::hacl::rsapss::hash(a, &mut (&mut m1)[8usize..], msgLen, msg);
    ((&mut m1)
        [8u32.wrapping_add(hLen) as usize..8u32.wrapping_add(hLen) as usize + saltLen as usize])
        .copy_from_slice(&salt[0usize..saltLen as usize]);
    crate::hacl::rsapss::hash(a, &mut m1Hash, m1Len, &m1);
    let emLen: u32 = emBits
        .wrapping_sub(1u32)
        .wrapping_div(8u32)
        .wrapping_add(1u32);
    ...
    ((&mut db)[last_before_salt.wrapping_add(1u32) as usize
        ..last_before_salt.wrapping_add(1u32) as usize + saltLen as usize])
        .copy_from_slice(&salt[0usize..saltLen as usize]);
    let mut dbMask: Box<[u8]> = vec![0u8; dbLen as usize].into_boxed_slice();
    crate::hacl::rsapss::mgf_hash(a, hLen, &m1Hash, dbLen, &mut dbMask);
    for i in 0u32..dbLen {
    ...
    (em[dbLen as usize..dbLen as usize + hLen as usize])
        .copy_from_slice(&(&m1Hash)[0usize..hLen as usize]);
...
fn pss_verify(
    ...
    let emLen: u32 = emBits
        ...
        .wrapping_add(1u32);
    let msBits: u32 = emBits.wrapping_rem(8u32);
    let em_0: u8 = if msBits > 0u32 {
        em[0usize] & 0xffu8.wrapping_shl(msBits)
    } else {
        0u8
    };
    ...
    if emLen
        < saltLen
            .wrapping_add(crate::hacl::rsapss::hash_len(a))
            .wrapping_add(2u32)
        || !(em_last == 0xbcu8 && em_0 == 0u8)
    {
        false
    } else {
        let emLen1: u32 = emBits
            .wrapping_sub(1u32)
            .wrapping_div(8u32)
            .wrapping_add(1u32);
        let hLen: u32 = crate::hacl::rsapss::hash_len(a);
        ...
        let mut dbMask: Box<[u8]> = vec![0u8; dbLen as usize].into_boxed_slice();
        crate::hacl::rsapss::mgf_hash(a, hLen, m1Hash.1, dbLen, &mut dbMask);
        for i in 0u32..dbLen {
        ...
        if z != 255u8 {
            ...
            let mut m1: Box<[u8]> = vec![0u8; m1Len as usize].into_boxed_slice();
            crate::hacl::rsapss::hash(a, &mut (&mut m1)[8usize..], msgLen, msg);
            ((&mut m1)[8u32.wrapping_add(hLen) as usize
                ..8u32.wrapping_add(hLen) as usize + saltLen as usize])
                .copy_from_slice(&salt.1[0usize..saltLen as usize]);
            crate::hacl::rsapss::hash(a, &mut m1Hash0, m1Len, &m1);
...
#[inline]
fn load_pkey(modBits: u32, eBits: u32, nb: &[u8], eb: &[u8], pkey: &mut [u64]) -> bool {
    let nbLen: u32 = modBits
        .wrapping_sub(1u32)
        .wrapping_div(8u32)
        .wrapping_add(1u32);
    let ebLen: u32 = eBits
    ...
    bignum::base::bn_precomp_r2_mod_n_u64(
        modBits
            .wrapping_sub(1u32)
            .wrapping_div(64u32)
            .wrapping_add(1u32),
    ...
    let m0: u64 = crate::hacl::rsapss::check_modulus_u64(modBits, r2.0);
    let m1: u64 = crate::hacl::rsapss::check_exponent_u64(eBits, e.1);
    let m: u64 = m0 & m1;
...
fn load_skey(
    ...
    let b: bool = crate::hacl::rsapss::load_pkey(modBits, eBits, nb, eb, d.0);
    bignum::bignum_base::bn_from_bytes_be_uint64(dbLen, db, d.1);
    let m1: u64 = crate::hacl::rsapss::check_exponent_u64(dBits, d.1);
...
pub fn rsapss_sign(
    ...
    let hLen: u32 = crate::hacl::rsapss::hash_len(a);
    let b: bool = saltLen <= 0xffffffffu32.wrapping_sub(hLen).wrapping_sub(8u32)
    ...
    if b {
        ...
        crate::hacl::rsapss::pss_encode(a, saltLen, salt, msgLen, msg, emBits, &mut em);
        bignum::bignum_base::bn_from_bytes_be_uint64(emLen, &em, &mut (&mut m)[0usize..]);
        let nLen1: u32 = modBits
            .wrapping_sub(1u32)
            .wrapping_div(64u32)
            .wrapping_add(1u32);
        let k: u32 = modBits
        ...
        let d: (&[u64], &[u64]) = e.1.split_at(
            nLen2.wrapping_add(nLen2).wrapping_add(eLen) as usize
                - nLen2.wrapping_add(nLen2) as usize,
        );
        let mu: u64 = bignum::base::mod_inv_uint64(r2.0[0usize]);
        bignum::base::bn_mod_exp_consttime_precomp_u64(
            modBits
                .wrapping_sub(1u32)
                .wrapping_div(64u32)
                .wrapping_add(1u32),
        ...
        let mu0: u64 = bignum::base::mod_inv_uint64(r2.0[0usize]);
        bignum::base::bn_mod_exp_vartime_precomp_u64(
            modBits
                .wrapping_sub(1u32)
                .wrapping_div(64u32)
                .wrapping_add(1u32),
...
pub fn rsapss_verify(
    ...
    if b {
        ...
        let res: bool = if mask == 0xFFFFFFFFFFFFFFFFu64 {
            let mu: u64 = bignum::base::mod_inv_uint64(r2.0[0usize]);
            bignum::base::bn_mod_exp_vartime_precomp_u64(
                modBits
                    .wrapping_sub(1u32)
        ...
        if b10 {
            ...
            let res0: bool = crate::hacl::rsapss::pss_verify(a, saltLen, msgLen, msg, emBits, &em);
            res0
        } else {
            false
        }
...
pub fn new_rsapss_load_skey(
    ...
    let ite0: bool = if ite && 0u32 < dBits {
        let nLen: u32 = modBits
            .wrapping_sub(1u32)
            .wrapping_div(64u32)
            .wrapping_add(1u32);
    ...
    if !ite0 {
        ...
        if false {
            ...
            let d: (&mut [u64], &mut [u64]) = pkey.1.split_at_mut(pkeyLen as usize);
            let nbLen1: u32 = modBits
                .wrapping_sub(1u32)
                .wrapping_div(8u32)
                .wrapping_add(1u32);
            let ebLen1: u32 = eBits
                .wrapping_sub(1u32)
                .wrapping_div(8u32)
                .wrapping_add(1u32);
            let nLen2: u32 = modBits
            ...
            let b: bool = m == 0xFFFFFFFFFFFFFFFFu64;
            bignum::bignum_base::bn_from_bytes_be_uint64(dbLen, db, d.1);
            let m10: u64 = crate::hacl::rsapss::check_exponent_u64(dBits, d.1);
...
pub fn rsapss_skey_sign(
    ...
    .into_boxed_slice();
    let b: bool = crate::hacl::rsapss::load_skey(modBits, eBits, dBits, nb, eb, db, &mut skey);
    if b {
        crate::hacl::rsapss::rsapss_sign(
            a, modBits, eBits, dBits, &skey, saltLen, salt, msgLen, msg, sgnt,
        )
    } else {
        false
    }
...
pub fn rsapss_pkey_verify(
    ...
    .into_boxed_slice();
    let b: bool = crate::hacl::rsapss::load_pkey(modBits, eBits, nb, eb, &mut pkey);
    if b {
        crate::hacl::rsapss::rsapss_verify(
            a, modBits, eBits, &pkey, saltLen, sgntLen, sgnt, msgLen, msg,
        )
    } else {
        false
    }
...
pub fn mgf_hash0(
    ...
    crate::hacl::rsapss::mgf_hash(a, len, mgfseed, maskLen, res)
}
