...
use libcrux_aes::aes_gcm_256::portable::PortableAesGcm256;
use libcrux_traits::aead::slice::Aead;
use libfuzzer_sys::fuzz_target;

fuzz_target!(|data: &[u8]| {
    if data.len() < 32 + 12 + 5 + 16 {
    ...
    let tag = &data[32 + 12 + 5..32 + 12 + 5 + 16];
    let ctxt = &data[32 + 12 + 5 + 16..];

    let mut ptxt = vec![0u8; ctxt.len()];

    // Decryption will mostly fail due to tag mismatch; we're checking for panics/crashes.
    let _ = PortableAesGcm256::decrypt(&mut ptxt, key, nonce, aad, ctxt, tag);
});
