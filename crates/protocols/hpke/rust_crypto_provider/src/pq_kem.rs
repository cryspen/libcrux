...
use ml_kem::{kem::Decapsulator, Decapsulate, Encapsulate, Generate, KeyExport, TryKeyInit};
...
pub(crate) fn kem_key_gen(
    ...
    match alg {
        ...
        KemAlgorithm::XWingDraft06 | KemAlgorithm::XWingDraft06Obsolete => {
            let dk = x_wing::DecapsulationKey::generate_from_rng(prng);
            let ek = dk.encapsulation_key();
            Ok((ek.to_bytes().as_slice().to_vec(), dk.as_bytes().to_vec()))
        }
        KemAlgorithm::MlKem768 => {
            let dk = ml_kem::DecapsulationKey768::generate_from_rng(prng);
            let ek_bytes = dk.encapsulation_key().to_bytes();
            let sk_bytes = dk.to_seed().ok_or(Error::KemInvalidSecretKey)?;
            Ok((ek_bytes.as_slice().to_vec(), sk_bytes.to_vec()))
        }
        KemAlgorithm::MlKem1024 => {
            let dk = ml_kem::DecapsulationKey1024::generate_from_rng(prng);
            let ek_bytes = dk.encapsulation_key().to_bytes();
            let sk_bytes = dk.to_seed().ok_or(Error::KemInvalidSecretKey)?;
            Ok((ek_bytes.as_slice().to_vec(), sk_bytes.to_vec()))
        }
...
pub(crate) fn kem_key_gen_derand(
    ...
    match alg {
        #[allow(deprecated)]
        KemAlgorithm::XWingDraft06 | KemAlgorithm::XWingDraft06Obsolete => {
            let seed: [u8; 32] = seed.try_into().map_err(|_| Error::KemInvalidSecretKey)?;
            let dk = x_wing::DecapsulationKey::from(seed);
            let ek = dk.encapsulation_key();
            Ok((ek.to_bytes().as_slice().to_vec(), dk.as_bytes().to_vec()))
        }
        KemAlgorithm::MlKem768 => {
            let seed = ml_kem::Seed::try_from(seed).map_err(|_| Error::KemInvalidSecretKey)?;
...
pub(crate) fn kem_encaps(
    ...
    match alg {
        ...
        KemAlgorithm::MlKem768 => {
            let ek_arr =
                ml_kem::array::Array::try_from(pk_r).map_err(|_| Error::KemInvalidPublicKey)?;
...
pub(crate) fn kem_decaps(alg: KemAlgorithm, ct: &[u8], sk_r: &[u8]) -> Result<Vec<u8>, Error> {
    match alg {
        #[allow(deprecated)]
        KemAlgorithm::XWingDraft06 | KemAlgorithm::XWingDraft06Obsolete => {
            let seed: [u8; 32] = sk_r.try_into().map_err(|_| Error::KemInvalidSecretKey)?;
            let dk = x_wing::DecapsulationKey::from(seed);
            let ct = x_wing::Ciphertext::try_from(ct).map_err(|_| Error::KemInvalidCiphertext)?;
            let ss = dk.decapsulate(&ct);
            Ok(ss.as_slice().to_vec())
        }
        KemAlgorithm::MlKem768 => {
            let seed = ml_kem::Seed::try_from(sk_r).map_err(|_| Error::KemInvalidSecretKey)?;
            let dk = ml_kem::DecapsulationKey768::from_seed(seed);
